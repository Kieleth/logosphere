...
def generate_registry_cpp(yaml_path: str, namespace: str, output_path: str):
    ...
    for cn in sorted(sv.all_classes()):
        ...
        lines.append(f'    reg.addEntityType("{cn}", "{parent}", {is_abstract});')
    ...
    for cn in sorted(sv.all_classes()):
        ...
        if ancestors:
            ...
            lines.append(f'    reg.addAncestors("{cn}", {anc_set});')
    ...
    for cn in sorted(sv.all_classes()):
        ...
        if facets:
            ...
            facet_lines.append(f'    reg.addFacets("{cn}", {fset});')
    if facet_lines:
        lines.append("    // Facets")
    ...
    for rel_name, (sources, targets) in sorted(rel_constraints.items()):
        ...
        lines.append(f'    reg.addRelationType("{rel_name}", {src_set}, {tgt_set});')
