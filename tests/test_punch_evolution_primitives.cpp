// Punch Evolution Primitives Test (FK Version)
//
// Tests FK-based arm/punch animation evolution from single joint
// to skill-parameterized cross punches.
...
int main(int, char**) {
    ...
    config.window_title = "FK Punch Evolution Test";
    ...
    // Create FK animation clips - arm/punch evolution, cycle through with SPACE
    std::vector<FKAnimationClip> clips;
    clips.push_back(create_fk_shoulder_abduction_right());    // Index 0: ARM (shoulder Y-axis)
    clips.push_back(create_fk_shoulder_then_elbow_right());   // Index 1: CHAINED (shoulder → elbow)
    clips.push_back(create_fk_cross_punch_windup_right());    // Index 2: WINDUP (T-pose → elbow flex → pull back)
    clips.push_back(create_fk_cross_punch_strike_right());    // Index 3: STRIKE (from windup → punch forward)
    clips.push_back(create_fk_cross_punch_full_right());     // Index 4: FULL PUNCH (windup + strike combined)
    clips.push_back(create_fk_cross_punch_full_recovery_right()); // Index 5: FULL PUNCH + RECOVERY
    clips.push_back(create_fk_guard_pose_right());                  // Index 6: GUARD POSE
    clips.push_back(create_fk_cross_punch_trained_right());         // Index 7: TRAINED CROSS PUNCH (spine torsion)
    // Skill-parameterized cross punches (continuous 0.0-1.0)
    // Store profiles for UI display and assertions
    struct SkillClipInfo {
        float skill;
        bool from_guard;
        CrossPunchProfile profile;
        const char* stage_name;     // Fitts & Posner stage
        const char* observe_hint;   // What to look for visually
    };
    std::vector<SkillClipInfo> skill_clips;
    auto add_skill_clip = [&](float skill, bool from_guard, const char* stage, const char* hint) {
        CrossPunchProfile profile = get_interpolated_profile(skill);
        clips.push_back(create_fk_cross_punch_right(skill, from_guard));
        skill_clips.push_back({skill, from_guard, profile, stage, hint});
    };
    size_t first_skill_index = clips.size();  // Index where skill clips start
    add_skill_clip(0.0f, false, "Cognitive",        "T-pose windup, arm-only, slow, wide");
    add_skill_clip(0.3f, true,  "Early Associative", "Loose guard, some hip, visible hip-arm lag");
    add_skill_clip(0.5f, true,  "Mid Associative",   "Interpolated - between 0.3 and 0.7");
    add_skill_clip(0.7f, true,  "Late Associative",  "Tight guard, near-auto, compact load");
    add_skill_clip(1.0f, true,  "Autonomous",        "Compact, explosive, full kinetic chain");
    ...
    // FK animation state - TEST_CASE env var selects which test (0-12), default 0
    ...
    std::cout << "[CONFIG] Running test case " << current_clip_index << " (set TEST_CASE=0-12 to change)" << std::endl;
    ...
    bool trained_punch_assertions_done = false;
    ...
    while (true) {
        ...
        if (fk_playing) {
            ...
            if (fk_time_ms > clip.duration_ms) {
                ...
                // === TC8: TRAINED CROSS PUNCH ASSERTIONS ===
                // Key differences vs novice:
                //   A: Max shoulder abduction never exceeds π/3 (compact, no T-pose)
                //   B: Strike keyframe gap ≤ 80ms (explosive, not sluggish)
                //   C: End pose has non-zero shoulder angles (guard, not rest)
                //   D: Chain connectivity at completion
                //   E: Spine joints return to zero after animation completes
                if (clip.name.find("trained_right") != std::string::npos &&
                    !trained_punch_assertions_done) {
                    trained_punch_assertions_done = true;
                    auto view = ps.lock_particles_for_read();

                    printf("\n[TC8 TRAINED PUNCH ANALYSIS]\n");

                    // TC8-A: Compact path — max abduction stayed within π/3 (60°)
                    // During the entire clip, shoulder abduction should not exceed π/3
                    // (trained punch stays compact, novice flares to π/2 T-pose)
                    // We check the clip keyframes directly for max abduction
                    float max_abduct = 0.0f;
                    for (const auto& kf : clip.keyframes) {
                        for (const auto& t : kf.pose.targets) {
                            if (t.joint_name == "right_shoulder" &&
                                t.semantic == SemanticChannel::ABDUCT) {
                                float abs_angle = std::abs(t.angle);
                                if (abs_angle > max_abduct) max_abduct = abs_angle;
                            }
                        }
                    }
                    printf("[TC8-A] Compact path — max shoulder abduction=%.3f (limit=%.3f) %s\n",
                           max_abduct, static_cast<float>(M_PI / 3.0),
                           max_abduct <= static_cast<float>(M_PI / 3.0) + 0.01f ? "PASS" : "FAIL");
                    if (max_abduct > static_cast<float>(M_PI / 3.0) + 0.01f) {
                        std::cerr << "[ASSERT FAIL TC8-A] Trained punch: shoulder abduction too wide!"
                                  << " max=" << max_abduct << " (limit π/3=" << static_cast<float>(M_PI / 3.0) << ")" << std::endl;
                        assertion_failures++;
                    }

                    // TC8-B: Strike speed — keyframe gap between load (250ms) and strike (310ms)
                    // Must be ≤ 80ms for trained (novice is 100ms+)
                    float load_time = -1.0f, strike_time = -1.0f;
                    for (size_t ki = 0; ki < clip.keyframes.size(); ki++) {
                        // Load is at 250ms, strike at 310ms by design
                        if (std::abs(clip.keyframes[ki].time_ms - 250.0f) < 1.0f) load_time = clip.keyframes[ki].time_ms;
                        if (std::abs(clip.keyframes[ki].time_ms - 310.0f) < 1.0f) strike_time = clip.keyframes[ki].time_ms;
                    }
                    float strike_gap = (load_time >= 0 && strike_time >= 0) ? (strike_time - load_time) : -1.0f;
                    printf("[TC8-B] Strike speed — gap=%.0fms (limit=80ms) %s\n",
                           strike_gap, (strike_gap > 0 && strike_gap <= 80.0f) ? "PASS" : "FAIL");
                    if (strike_gap > 80.0f) {
                        std::cerr << "[ASSERT FAIL TC8-B] Trained punch: strike too slow!"
                                  << " gap=" << strike_gap << "ms (limit 80ms)" << std::endl;
                        assertion_failures++;
                    }

                    // TC8-C: Guard end pose — shoulder angles non-zero at clip end
                    // Last keyframe should be guard position (non-rest)
                    if (!clip.keyframes.empty()) {
                        const auto& last_kf = clip.keyframes.back();
                        bool has_shoulder_angle = false;
                        for (const auto& t : last_kf.pose.targets) {
                            if (t.joint_name == "right_shoulder" && std::abs(t.angle) > 0.01f) {
                                has_shoulder_angle = true;
                                break;
                            }
                        }
                        printf("[TC8-C] Guard end pose — has_shoulder_angle=%s %s\n",
                               has_shoulder_angle ? "YES" : "NO",
                               has_shoulder_angle ? "PASS" : "FAIL");
                        if (!has_shoulder_angle) {
                            std::cerr << "[ASSERT FAIL TC8-C] Trained punch ends at rest, not guard!" << std::endl;
                            assertion_failures++;
                        }
                    }

                    // TC8-D: Connectivity — right arm chain intact
                    {
                        auto validation = validate_humanoid(ps, physics, h);
                        printf("\n[TC8 TRAINED PUNCH CONNECTIVITY]\n");
                        validation.print_report();
                        float arm_err = validation.chain_max_error("r_");
                        printf("[TC8-D] Chain error — r_arm max_err=%.4f %s\n", arm_err,
                               arm_err < 0.02f ? "PASS" : "FAIL");
                        if (arm_err > 0.02f) {
                            std::cerr << "[ASSERT FAIL TC8-D] Trained punch: right arm chain disconnected!"
                                      << " max_pivot_err=" << arm_err << std::endl;
                            assertion_failures++;
                        }
                    }

                    // TC8-E: Spine joints return to zero after animation
                    // Last keyframe should have lower_spine and upper_spine twist = 0
                    if (!clip.keyframes.empty()) {
                        const auto& last_kf = clip.keyframes.back();
                        bool spine_neutral = true;
                        for (const auto& t : last_kf.pose.targets) {
                            if ((t.joint_name == "lower_spine" || t.joint_name == "upper_spine") &&
                                t.semantic == SemanticChannel::TWIST &&
                                std::abs(t.angle) > 0.01f) {
                                spine_neutral = false;
                            }
                        }
                        printf("[TC8-E] Spine neutral at end — %s\n",
                               spine_neutral ? "PASS" : "FAIL");
                        if (!spine_neutral) {
                            std::cerr << "[ASSERT FAIL TC8-E] Trained punch: spine not neutral at end!" << std::endl;
                            assertion_failures++;
                        }
                    }
                }

                // === SKILL PUNCH ASSERTIONS (tests 8-12) ===
                // Verifies:
                //   A: Chain connectivity at completion
                //   B: Spine returns to neutral
                //   C: Guard end-pose (from_guard clips have non-zero shoulder angles)
                //   D: Duration monotonicity (higher skill = shorter or equal)
                bool completed_is_skill = (clip.name.find("cross_punch_skill_") != std::string::npos);
                if (completed_is_skill && completed_index >= first_skill_index) {
                    size_t skill_idx = completed_index - first_skill_index;
                    const auto& info = skill_clips[skill_idx];

                    printf("\n[SKILL PUNCH ANALYSIS] skill=%.1f stage=%s from_guard=%s duration=%.0fms\n",
                           info.skill, info.stage_name, info.from_guard ? "YES" : "NO", clip.duration_ms);

                    // SKILL-A: Chain connectivity
                    // Threshold 0.05 (5cm) — looser than legacy clips (0.02) because
                    // generated keyframe structure differs from hand-crafted, affecting
                    // final settling. Still catches gross disconnection (>5cm).
                    {
                        auto validation = validate_humanoid(ps, physics, h);
                        float arm_err = validation.chain_max_error("r_");
                        printf("[SKILL-A] Chain connectivity — r_arm max_err=%.4f %s\n",
                               arm_err, arm_err < 0.05f ? "PASS" : "FAIL");
                        if (arm_err > 0.05f) {
                            std::cerr << "[ASSERT FAIL SKILL-A] Skill " << info.skill
                                      << ": right arm chain disconnected! max_err=" << arm_err << std::endl;
                            assertion_failures++;
                        }
                    }

                    // SKILL-B: Spine neutral at end
                    if (!clip.keyframes.empty()) {
                        const auto& last_kf = clip.keyframes.back();
                        bool spine_neutral = true;
                        for (const auto& t : last_kf.pose.targets) {
                        ...
                        printf("[SKILL-B] Spine neutral at end — %s\n",
                               spine_neutral ? "PASS" : "FAIL");
                        if (!spine_neutral) {
                            std::cerr << "[ASSERT FAIL SKILL-B] Skill " << info.skill
                                      << ": spine not neutral at end!" << std::endl;
                            assertion_failures++;
                        }
                    }

                    // SKILL-C: Guard end-pose (from_guard clips end in guard, not rest)
                    if (info.from_guard && !clip.keyframes.empty()) {
                        const auto& last_kf = clip.keyframes.back();
                        bool has_shoulder_angle = false;
                        for (const auto& t : last_kf.pose.targets) {
                            if (t.joint_name == "right_shoulder" && std::abs(t.angle) > 0.01f) {
                        ...
                        printf("[SKILL-C] Guard end-pose — has_shoulder_angle=%s %s\n",
                               has_shoulder_angle ? "YES" : "NO",
                               has_shoulder_angle ? "PASS" : "FAIL");
                        if (!has_shoulder_angle) {
                            std::cerr << "[ASSERT FAIL SKILL-C] Skill " << info.skill
                                      << ": from_guard clip ends at rest, not guard!" << std::endl;
                            assertion_failures++;
                        }
                    }

                    // SKILL-D: Duration monotonicity (each higher skill should be <= previous)
                    if (skill_idx > 0) {
                        float prev_duration = clips[first_skill_index + skill_idx - 1].duration_ms;
                        float this_duration = clip.duration_ms;
                        // Allow 50ms tolerance for guard/no-guard difference at low skill
                        bool monotonic = this_duration <= prev_duration + 50.0f;
                        printf("[SKILL-D] Duration monotonicity — prev=%.0fms this=%.0fms %s\n",
                               prev_duration, this_duration, monotonic ? "PASS" : "FAIL");
                        if (!monotonic) {
                            std::cerr << "[ASSERT FAIL SKILL-D] Skill " << info.skill
                    ...
                    // SKILL-E: Strike speed scales with skill
                    // Load+strike time should decrease as skill increases
                    float load_strike_ms = info.profile.load_ms + info.profile.strike_ms;
                    printf("[SKILL-E] Load+strike timing — %.0fms (load=%.0f strike=%.0f)\n",
                           load_strike_ms, info.profile.load_ms, info.profile.strike_ms);
                }
            ...
            bool is_trained_punch = (clip.name.find("trained_right") != std::string::npos);
            bool is_skill_punch = (clip.name.find("cross_punch_skill_") != std::string::npos);

            // Track ALL arm animations (test 1, 2, 3, 4, 5, 7, 8-12)
            if ((is_chained || is_windup || is_strike || is_full_punch || is_recovery_punch || is_trained_punch || is_skill_punch) && shoulder_id >= 0 && upper_arm_id >= 0 && forearm_id >= 0) {
            ...
            bool collect_samples = (is_windup && fk_time_ms >= 300.0f && fk_time_ms <= 600.0f) ||
                                   ...
                                   (is_trained_punch && fk_time_ms >= 100.0f && fk_time_ms <= 810.0f) ||
                                   (is_skill_punch && fk_time_ms >= 0.0f && fk_time_ms <= clip.duration_ms);
            if (collect_samples) {
                auto view = ps.lock_particles_for_read();
        ...
        // Extended UI for skill-parameterized clips (indices 8-12)
        if (current_clip_index >= first_skill_index &&
            ...
            // Skill level and motor learning stage
            char buf[128];
            snprintf(buf, sizeof(buf), "Skill: %.1f  Stage: %s", sci.skill, sci.stage_name);
            rs.draw_text(20, 75, buf, 180, 220, 255);

            // Stance context
            snprintf(buf, sizeof(buf), "Stance: %s", sci.from_guard ? "FROM GUARD" : "FROM REST");
            rs.draw_text(20, 95, buf, 180, 220, 255);

            // Key biomechanical parameters
            snprintf(buf, sizeof(buf), "Load: %.0fms  Strike: %.0fms  Total: %.0fms",
                     sci.profile.load_ms, sci.profile.strike_ms, clips[current_clip_index].duration_ms);
            rs.draw_text(20, 120, buf, 200, 200, 200);

            snprintf(buf, sizeof(buf), "Hip-arm gap: %.0fms  Spine: %.1fdeg",
                     sci.profile.hip_arm_gap_ms,
                     sci.profile.spine_lower_twist * 180.0f / static_cast<float>(M_PI));
            rs.draw_text(20, 140, buf, 200, 200, 200);

            snprintf(buf, sizeof(buf), "Pronation: %.1fdeg  Recovery flare: %.1fdeg",
                     sci.profile.strike_shoulder_twist * 180.0f / static_cast<float>(M_PI),
                     sci.profile.recovery_abduct_flare * 180.0f / static_cast<float>(M_PI));
            ...
            // Current phase indicator (based on elapsed time)
            if (fk_playing) {
                const char* phase = "IDLE";
                float t_acc = 0.0f;
                float tpose_setup = (!sci.from_guard && sci.skill < 0.01f) ? 400.0f : 0.0f;
                t_acc += tpose_setup;
                if (fk_time_ms <= t_acc) { phase = tpose_setup > 0 ? "T-POSE SETUP" : "START"; }
                else {
                    t_acc += sci.profile.guard_settle_ms;
                    if (fk_time_ms <= t_acc) { phase = "GUARD SETTLE"; }
                    else {
                        float eff_load = sci.profile.load_ms;
                        if (sci.from_guard && sci.skill < 0.3f) eff_load *= 1.2f;
                        t_acc += eff_load;
                        if (fk_time_ms <= t_acc) { phase = "LOAD (WINDUP)"; }
                        else {
                            if (sci.profile.hip_arm_gap_ms > 0.5f) {
                                t_acc += sci.profile.hip_arm_gap_ms;
                                if (fk_time_ms <= t_acc) { phase = "HIP FIRES"; }
                            }
                            if (fk_time_ms > t_acc) {
                                t_acc += sci.profile.strike_ms;
                                if (fk_time_ms <= t_acc) { phase = ">>> STRIKE <<<"; }
                                else {
                ...
                // Highlight strike phase in red
                bool is_strike_phase = (std::string(phase).find("STRIKE") != std::string::npos);
                rs.draw_text(20, 210, buf,
                             is_strike_phase ? (uint8_t)255 : (uint8_t)100,
                             is_strike_phase ? (uint8_t)80 : (uint8_t)255,
                             is_strike_phase ? (uint8_t)80 : (uint8_t)100);
            }

            // Assertions summary (what we're testing)
            rs.draw_text(20, 240, "Assertions: connectivity, spine neutral,", 140, 140, 140);
            rs.draw_text(20, 258, "guard end-pose, duration monotonicity", 140, 140, 140);
        }
