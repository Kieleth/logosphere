...
extern bool test_plants_are_rooted();         // ISSUE #47: plants must be attached to the ground (STANDALONE)
...
extern bool test_drive_gravity_ff();  // PHYSICS-DRIVE: gravity compensation ladder (task #41, STANDALONE)
...
static std::unordered_map<std::string, std::function<bool(TestContext&)>> create_test_registry() {
    ...
    registry["test_plants_are_rooted"] = [](TestContext&) { return test_plants_are_rooted(); };  // STANDALONE: own engine
    ...
    registry["test_drive_gravity_ff"] = [](TestContext&) { return test_drive_gravity_ff(); };  // PHYSICS-DRIVE: gravity compensation ladder (STANDALONE)
...
static const std::unordered_map<std::string, std::function<bool()>>& get_standalone_registry() {
    static const std::unordered_map<std::string, std::function<bool()>> reg = {
        ...
        {"test_plants_are_rooted", test_plants_are_rooted},
