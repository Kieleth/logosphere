// sphere_lighting_test — minimal repro for the "lit rim, dark interior" bug.
// ONE sphere, ONE movable light, ONE pad.
//
// Controls:
...
constexpr float kSphereRadius = 0.50f;
constexpr float kSphereCenter = kSphereRadius;           // bottom touches pad top
...
private:
    ...
    void print_telemetry() const {
        ...
        std::printf("[telem] expected per-surface brightness (6 cardinal points on the SPHERE):\n");

        struct P { const char* name; float nx, ny, nz; };
        const P sphere_pts[] = {
        ...
        for (const auto& p : sphere_pts) {
            float wx = p.nx * kSphereRadius;
            float wy = p.ny * kSphereRadius;
            float wz = kSphereCenter + p.nz * kSphereRadius;
            print_one(p.name, wx, wy, wz, p.nx, p.ny, p.nz, L.x, L.y, L.z,
                      0.15f, 0.95f, 1.00f);
        }
