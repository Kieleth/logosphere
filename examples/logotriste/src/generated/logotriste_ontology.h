...
/// The seven free tetrominoes, named for the letters they resemble.
enum class TetrominoKind {
    /// Four cells in a line.
    I,
    /// The square.
    O,
    /// Three in a row plus one centred above.
    T,
    /// Two offset pairs leaning right.
    S,
    /// Two offset pairs leaning left.
    Z,
    /// Three in a row plus one above the left end.
    J,
    /// Three in a row plus one above the right end.
    L
};

/// Convert TetrominoKind to its string representation.
inline const char* to_string(TetrominoKind value) {
    switch (value) {
        case TetrominoKind::I: return "I";
        case TetrominoKind::O: return "O";
        case TetrominoKind::T: return "T";
        case TetrominoKind::S: return "S";
        case TetrominoKind::Z: return "Z";
        case TetrominoKind::J: return "J";
        case TetrominoKind::L: return "L";
    }
    return "unknown";
}

/// Parse a string into TetrominoKind. Returns false if the string is not a valid value.
inline bool from_string(const char* str, TetrominoKind& out) {
    if (std::strcmp(str, "I") == 0) { out = TetrominoKind::I; return true; }
    if (std::strcmp(str, "O") == 0) { out = TetrominoKind::O; return true; }
    if (std::strcmp(str, "T") == 0) { out = TetrominoKind::T; return true; }
    if (std::strcmp(str, "S") == 0) { out = TetrominoKind::S; return true; }
    if (std::strcmp(str, "Z") == 0) { out = TetrominoKind::Z; return true; }
    if (std::strcmp(str, "J") == 0) { out = TetrominoKind::J; return true; }
    if (std::strcmp(str, "L") == 0) { out = TetrominoKind::L; return true; }
    return false;
}
...
/// The playfield. Its frame is drawn from its own particles.
struct LogotristeWell : public WorldEntity {
    /// Width of the well in cells.
    std::optional<int32_t> well_columns = std::nullopt;
    /// Height of the well in cells.
    std::optional<int32_t> well_rows = std::nullopt;
};


/// One locked cell resting in the well.
struct LogotristeMino : public WorldEntity {
    /// Column index, 0 at the left wall.
    std::optional<int32_t> cell_column = std::nullopt;
    /// Row index, 0 at the floor.
    std::optional<int32_t> cell_row = std::nullopt;
    /// Which of the seven pieces this cell came from.
    std::optional<TetrominoKind> tetromino_kind = std::nullopt;
};


/// The tetromino currently falling.
struct LogotristePiece : public WorldEntity {
    /// Column index, 0 at the left wall.
    std::optional<int32_t> cell_column = std::nullopt;
    /// Row index, 0 at the floor.
    std::optional<int32_t> cell_row = std::nullopt;
    /// Which of the seven pieces this cell came from.
    std::optional<TetrominoKind> tetromino_kind = std::nullopt;
    /// Rotation state, 0 to 3, clockwise.
    std::optional<int32_t> piece_rotation = std::nullopt;
};
