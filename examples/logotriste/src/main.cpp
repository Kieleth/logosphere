// Logotriste on Logosphere — driver.
//
...
//
//   ./build/logotriste/logotriste                     windowed
//   ./build/logotriste/logotriste --no-head --exit-after 5 --seed 7   headless
...
static bool write_shot(Engine& engine, const std::string& path) {
    ...
    if (!engine.read_latest_framebuffer(px.data(), gw, gh)) {
        std::fprintf(stderr, "[logotriste] read_latest_framebuffer failed\n");
        return false;
    }
    ...
    std::fclose(f);
    std::printf("[logotriste] wrote %s (%dx%d)\n", path.c_str(), gw, gh);
    return true;
}
...
int main(int argc, char* argv[]) {
    ...
    logotriste::LogotristeApplication app;
    ...
    config.window_title = "Logotriste";
    config.show_debug_overlay = false;
    config.show_kg_inspector = false;
    config.enable_chat_window = false;

    if (engine.initialize(config) < 0) {
        std::fprintf(stderr, "[logotriste] engine.initialize() failed\n");
        return 1;
    }

    if (headless && exit_after <= 0.0) {
        std::fprintf(stderr,
                     "[logotriste] headless without --exit-after would never "
                     "return; using 10 s\n");
        exit_after = 10.0;
    }
    ...
    while (headless ? true : engine.should_continue()) {
        ...
        if (headless && !shooting && app.over()) {
            ...
            if (test_restart && !restarted) {
                std::printf("[logotriste] restart probe: score before=%d lines=%d\n",
                            app.score(), app.lines());
                app.handle_key(GLFW_KEY_R, 0, GLFW_PRESS, 0);
                restarted = true;
                if (app.over() || app.score() != 0 || app.lines() != 0) {
                    std::fprintf(stderr, "[logotriste] RESTART FAILED: over=%d "
                                 "score=%d lines=%d\n",
    ...
    std::printf("[logotriste] final: score=%d lines=%d level=%d over=%s\n",
                app.score(), app.lines(), app.level(),
                app.over() ? "yes" : "no");
    std::printf("[logotriste] kg: entities=%zu relations=%zu bytes=%zu | particles=%zu\n",
                st.entity_count, st.relation_count, st.memory_bytes,
                engine.get_particle_system().count());
    if (test_restart) {
        std::printf("[logotriste] restart probe: %s (played on to score=%d lines=%d)\n",
