...
// locked cell is a LogotristeMino entity in the knowledge graph and the
// falling piece is a LogotristePiece entity; Well::cells_ holds their ids,
...
#include "logotriste_ontology_registry.h"
#include "tetromino.h"
#include "well.h"
...
class LogotristeApplication : public Logosphere::MacOSPlatform {
public:
    LogotristeApplication() {
        set_app_name("Logotriste - Logosphere");
    ...
    void initialize_game(void* engine_ptr) override {
        ...
        kg.extendOntology(logotriste::ontology::registry());

        engine_->set_projection_mode(ProjectionMode::BirdsEye);
        frame_camera();

        well_entity_ = kg.createEntity("LogotristeWell");
        ...
        rng_.seed(seed_);
        std::printf("[logotriste] seed=%u\n", seed_);

        refill_bag();
        spawn_next();

        std::printf(
            "[logotriste] LEFT/RIGHT move, UP or X rotate CW, Z rotate CCW,\n"
            "[logotriste] DOWN soft drop, SPACE hard drop, R restart, ESC quit.\n");
    }
    ...
    void restart() {
        ...
        refill_bag();
        spawn_next();
        std::printf("[logotriste] restarted\n");
    }
...
private:
    ...
    void build_hud() {
        ...
        const int x = 24;
        int y = 24;
        const char* ids[kHudLines] = {"logotriste_score", "logotriste_lines",
                                      "logotriste_level", "logotriste_state"};
    ...
    void spawn_next() {
        ...
        if (piece_entity_ == kg::INVALID_ENTITY) {
            piece_entity_ = kg.createEntity("LogotristePiece");
        }
        kg.setProperty(piece_entity_, "tetromino_kind", kind_name(kind_));

        if (!well_.fits(kind_, rot_, px_, py_)) {
            game_over_ = true;
            std::printf("[logotriste] TOP OUT — score %d, lines %d, level %d\n",
                        score_, lines_, level());
        }
        sync_piece();
    }
    ...
    void lock_piece() {
        ...
        for (const Cell& c : kShapes[static_cast<int>(kind_)][rot_]) {
            ...
            kg::EntityID mino = kg.createEntity("LogotristeMino");
        ...
        if (!full.empty()) {
            ...
            std::printf("[logotriste] cleared %d — score %d, lines %d, level %d\n",
