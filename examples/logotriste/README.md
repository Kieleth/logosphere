...
Logotriste on Logosphere. Ten columns, twenty rows, seven pieces, seen
from directly above.

```bash
cmake -S . -B build -DCMAKE_OSX_ARCHITECTURES="arm64"
cmake --build build --target logotriste test_logotriste_well -j
./build/logotriste/logotriste                 # play
./build/test_logotriste_well              # the rules, headless
```
...
`schema/logotriste.yaml` declares three types and one enum:

| Type | One per | Carries |
|---|---|---|
| `LogotristeWell` | the game | `well_columns`, `well_rows`, the frame particles |
| `LogotristePiece` | the game | the falling tetromino's kind, column, row, rotation |
| `LogotristeMino` | locked cell | its column, row, and which piece it came from |
...
`tests/test_logotriste_well.cpp` is pure C++17 — no engine, no window, no
