// Pacman on Logosphere — entry point and main loop.
//
//   ./build/pacman/pacman                 windowed
//   ./build/pacman/pacman --no-head --exit-after 20
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "PACMAN on Logosphere" << std::endl;
    ...
    if (engine.initialize(config) < 0) {
        std::cerr << "[pacman] engine.initialize() failed" << std::endl;
        return 1;
    }
    ...
    std::cout << "[pacman] final score " << app.score()
              << ", lives " << app.lives()
