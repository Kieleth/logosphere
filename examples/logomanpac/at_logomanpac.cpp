// Logomanpac acceptance test — headless, and it plays the game.
//
// This drives the SAME LogomanpacApplication the window drives
...
void run(Engine& e, LogomanpacApplication& app, float seconds, float step = 1.0f / 60.0f) {
    (void)app;
...
struct Leg { int col; int row; logomanpac::Dir then; };

void follow_route(Engine& e, LogomanpacApplication& app,
    ...
    for (int i = 0; i < frames && next < legs.size(); ++i) {
        ...
        const int c = logomanpac::cell_of(av.col);
        const int r = logomanpac::cell_of(av.row);
        if (c == legs[next].col && r == legs[next].row &&
            logomanpac::at_cell_centre(av, 0.14f)) {
...
int main() {
    std::cout << "at_logomanpac — headless Logomanpac, playing itself" << std::endl;
    ...
    cfg.window_title = "at_logomanpac";
    ...
    run(engine, app, logomanpac::kReadySecs + 0.2f);   // burn the READY pause
    check(app.phase() == logomanpac::Phase::PLAYING, "round went live");
    ...
    // (There is no way to stand still in Logomanpac on purpose; a wall is
    ...
    std::cout << "\n[4] the energizer flips every ghost" << std::endl;
    {
        ...
        LogomanpacApplication e_app;
        Engine e_engine(&e_app);
        EngineConfig ecfg;
        ecfg.create_display = false;
        ecfg.window_title = "at_logomanpac_energizer";
        e_engine.initialize(ecfg);
        run(e_engine, e_app, logomanpac::kReadySecs + 0.1f);
        ...
        follow_route(e_engine, e_app, {
            {13, 23, logomanpac::Dir::LEFT},
            { 6, 23, logomanpac::Dir::DOWN},
            { 6, 26, logomanpac::Dir::LEFT},
            { 3, 26, logomanpac::Dir::UP},
            { 3, 23, logomanpac::Dir::LEFT},
        }, /*budget_s=*/8.0f);
        run(e_engine, e_app, 1.0f);
        int frightened = 0;
        for (const auto& gh : e_app.ghosts())
            if (gh.mood == logomanpac::Mood::FRIGHTENED) ++frightened;
        ...
        check(e_app.score() >= logomanpac::kEnergizerPoints,
              "the energizer paid its 50 points");

        // and it wears off
        run(e_engine, e_app, logomanpac::kFrightenedSecs + 1.0f);
        int still = 0;
        for (const auto& gh : e_app.ghosts())
            if (gh.mood == logomanpac::Mood::FRIGHTENED) ++still;
        std::cout << "  [measure] after " << logomanpac::kFrightenedSecs + 1.0f
    ...
    {
        logomanpac::Walker w;
        w.col = 2.0f; w.row = static_cast<float>(logomanpac::kTunnelRow);
        w.dir = logomanpac::Dir::LEFT; w.speed = 8.0f;
        float min_col = w.col, max_col = w.col;
        for (int i = 0; i < 120; ++i) {
            logomanpac::step_walker(w, board, 1.0f / 60.0f, false);
        ...
        c.dir = logomanpac::Dir::LEFT; c.speed = 8.0f;
        for (int i = 0; i < 120; ++i) logomanpac::step_walker(c, board, 1.0f / 60.0f, false);
    ...
    {
        LogomanpacApplication h_app;
        Engine h_engine(&h_app);
        EngineConfig hcfg;
        hcfg.create_display = false;
        hcfg.window_title = "at_logomanpac_house";
        ...
        auto outside_house = [](const logomanpac::GhostSlot& gh) {
            return gh.walker.row < logomanpac::kHouseDoorRow + 0.5f ||
                   gh.walker.row > 16.5f ||
        ...
        for (int i = 0; i < static_cast<int>(30.0f / step); ++i) {
            ...
            if (h_app.phase() == logomanpac::Phase::GAME_OVER)
    ...
    {
        LogomanpacApplication b_app;
        Engine b_engine(&b_app);
        EngineConfig bcfg;
        bcfg.create_display = false;
        bcfg.window_title = "at_logomanpac_bot";
        ...
        for (int i = 0; i < frames; ++i) {
            ...
            if (logomanpac::at_cell_centre(av, 0.12f)) {
                const int c = b.wrap_col(logomanpac::cell_of(av.col));
                const int r = logomanpac::cell_of(av.row);
                logomanpac::Dir opts[4];
                int n = 0;
                const logomanpac::Dir all[4] = {logomanpac::Dir::UP, logomanpac::Dir::DOWN,
                                            logomanpac::Dir::LEFT, logomanpac::Dir::RIGHT};
                for (logomanpac::Dir d : all) {
                    if (d == logomanpac::opposite(av.dir)) continue;
                    int dc, dr; logomanpac::dir_delta(d, dc, dr);
                    if (b.open_for_avatar(c + dc, r + dr)) opts[n++] = d;
                }
                if (n == 0) b_app.queue(logomanpac::opposite(av.dir));
                else if (n == 1) b_app.queue(opts[0]);
                else if (av.dir == logomanpac::Dir::NONE ||
                         (logomanpac::next_random(rng) % 100u) < 45u)
                    b_app.queue(opts[logomanpac::next_random(rng) % static_cast<uint32_t>(n)]);
            }
            ...
            if (b_app.phase() == logomanpac::Phase::GAME_OVER)
            ...
            const int ac = b.wrap_col(logomanpac::cell_of(b_app.avatar().col));
            const int ar = logomanpac::cell_of(b_app.avatar().row);
    ...
    if (std::getenv("LOGOSPHERE_VISUAL")) {
        ...
        for (int i = 0; i < warm_frames + 45; ++i) {
            ...
            if (i < warm_frames && logomanpac::at_cell_centre(av, 0.12f)) {
                const auto& vb = v_app.board();
                const int c = vb.wrap_col(logomanpac::cell_of(av.col));
                const int r = logomanpac::cell_of(av.row);
                logomanpac::Dir opts[4]; int n = 0;
                const logomanpac::Dir all[4] = {logomanpac::Dir::UP, logomanpac::Dir::DOWN,
                                            logomanpac::Dir::LEFT, logomanpac::Dir::RIGHT};
                for (logomanpac::Dir d : all) {
                    if (d == logomanpac::opposite(av.dir)) continue;
                ...
                if (n > 0 && (av.dir == logomanpac::Dir::NONE ||
                              (logomanpac::next_random(vrng) % 100u) < 45u))
                    v_app.queue(opts[logomanpac::next_random(vrng) % static_cast<uint32_t>(n)]);
            }
        ...
        // hardest to notice — a red maze with a cyan Logomanpac still reads
        // as "a maze with a Logomanpac in it".
