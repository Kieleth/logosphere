...
cmake --build build --target logomanpac at_logomanpac -j

./build/logomanpac/logomanpac                       # play
./build/logomanpac/logomanpac --no-head --exit-after 20   # no window, same logic
./build/at_logomanpac                           # headless acceptance test
LOGOSPHERE_VISUAL=1 ./build/at_logomanpac       # + framebuffer pixel assertions
```
...
lighting. Logomanpac is a flat 2D game; with it set, the whole thing needs
...
| `src/logomanpac_app.h` | engine, KG | The same rules bound to entities and particles. |
| `src/main.cpp` | the above | Argument parsing and the loop. |
| `at_logomanpac.cpp` | the above | Plays the game headless. |
...
`at_logomanpac` drives the same `LogomanpacApplication` the window drives — not
...
backwards produces a red maze with a cyan Logomanpac, which still looks
enough like a Logomanpac to pass an eyeball check.
