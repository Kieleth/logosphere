...
// needs no lights, no sun and no shadow rig. Logomanpac is a 2D game; a
...
#include "maze.h"
#include "logomanpac_ontology_registry.h"
...
class LogomanpacApplication : public Logosphere::IApplication {
public:
    // ---- IApplication ------------------------------------------------
    bool initialize() override { return true; }
    void shutdown() override {}
    GLFWwindow* get_window() override { return nullptr; }
    const char* get_app_name() const override { return "Logomanpac"; }
    ...
    void initialize_game(void* engine_ptr) override {
        ...
        kg.extendOntology(logomanpac::ontology::registry());
        ...
        std::cout << "[logomanpac] board " << board_.cols() << "x" << board_.rows()
                  << ", " << board_.pellet_count() << " pellets, "
                  << problems.size() << " defect(s)" << std::endl;
        for (const auto& p : problems)
            std::cerr << "[logomanpac] BOARD DEFECT: " << p << std::endl;
        ...
        std::cout << "[logomanpac] controls: arrows or WASD to steer, "
    ...
    bool handle_key(int key, int /*scancode*/, int action, int /*mods*/) override {
        ...
        switch (key) {
            case GLFW_KEY_UP:    case GLFW_KEY_W: queue(logomanpac::Dir::UP);    return true;
            case GLFW_KEY_DOWN:  case GLFW_KEY_S: queue(logomanpac::Dir::DOWN);  return true;
            case GLFW_KEY_LEFT:  case GLFW_KEY_A: queue(logomanpac::Dir::LEFT);  return true;
            case GLFW_KEY_RIGHT: case GLFW_KEY_D: queue(logomanpac::Dir::RIGHT); return true;
    ...
    logomanpac::Phase phase()     const { return phase_; }
    bool  board_ok()          const { return board_ok_; }
    const logomanpac::Board& board() const { return board_; }
    const logomanpac::Walker& avatar() const { return avatar_; }
    const std::vector<logomanpac::GhostSlot>& ghosts() const { return ghosts_; }
    ...
    void queue(logomanpac::Dir d) { avatar_.desired = d; }
...
private:
    ...
    void build_walls(kg::KGModule& kg) {
        ...
        for (int r = 0; r < board_.rows(); ++r) {
            for (int c = 0; c < board_.cols(); ++c) {
                const logomanpac::Tile t = board_.at(c, r);
                if (t != logomanpac::Tile::WALL && t != logomanpac::Tile::DOOR) continue;
                ...
                p.z = logomanpac::kWallZ;
                if (t == logomanpac::Tile::DOOR) {
        ...
        std::cout << "[logomanpac] " << made << " wall cells" << std::endl;
    }
    ...
    void build_pellets(kg::KGModule& kg) {
        ...
        for (int r = 0; r < board_.rows(); ++r) {
            for (int c = 0; c < board_.cols(); ++c) {
                const logomanpac::Tile t = board_.at(c, r);
                const bool energizer = (t == logomanpac::Tile::ENERGIZER);
                if (t != logomanpac::Tile::PELLET && !energizer) continue;

                logomanpac::PelletSlot slot;
                ...
                kg.setProperty(slot.entity, "points",
                               std::to_string(energizer ? logomanpac::kEnergizerPoints
                                                        : logomanpac::kPelletPoints));

                Particle p{};
                p.shape = ParticleShape::SPHERE;
                board_.cell_to_world(static_cast<float>(c),
                                     static_cast<float>(r), p.x, p.y);
                p.z = logomanpac::kPelletZ;
        ...
        std::cout << "[logomanpac] " << pellets_.size() << " pellets laid" << std::endl;
    }
    ...
    void build_ghosts(kg::KGModule& kg) {
        ...
        const Spec specs[4] = {
            ...
            {"BLINKY", 1.00f, 0.16f, 0.16f, board_.cols() - 2, 0,
             logomanpac::kHouseExitCol, logomanpac::kHouseExitRow, 0.0f, false},
        ...
        for (const auto& s : specs) {
            ...
            gs.walker.dir = logomanpac::Dir::LEFT;
            gs.walker.speed = logomanpac::kGhostSpeed;
    ...
    void setup_camera() {
        ...
        cam.set_position(0.0f, logomanpac::kBoardOffsetY, 30.0f);
        cam.look_at(0.0f, logomanpac::kBoardOffsetY, 0.0f);
        fit_camera_zoom();
    }
    ...
    void setup_hud() {
        ...
        hud_ = std::make_unique<TextWindow>("LOGOMANPAC", "logomanpac_hud");
    ...
    void reset_round(bool full_restart) {
        if (full_restart) {
            score_ = 0;
            lives_ = logomanpac::kStartingLives;
            deaths_total_ = 0;
            ghosts_eaten_total_ = 0;
            relay_all_pellets();
        }
        avatar_.col = static_cast<float>(logomanpac::kAvatarStartCol);
        avatar_.row = static_cast<float>(logomanpac::kAvatarStartRow);
        avatar_.dir = logomanpac::Dir::NONE;
        avatar_.desired = logomanpac::Dir::NONE;
        avatar_.speed = logomanpac::kAvatarSpeed;

        const int starts[4][2] = {{logomanpac::kHouseExitCol, logomanpac::kHouseExitRow},
                                  {13, 14}, {11, 14}, {16, 14}};
        ...
        for (size_t i = 0; i < ghosts_.size(); ++i) {
            ...
            gh.walker.dir = logomanpac::Dir::LEFT;
            gh.walker.desired = logomanpac::Dir::NONE;
            gh.walker.speed = logomanpac::kGhostSpeed;
            gh.mood = logomanpac::Mood::SCATTER;
            gh.in_house = houses[i];
            gh.release_at = rel[i];
            gh.frightened_left = 0.0f;
        }
        phase_ = logomanpac::Phase::READY;
        phase_timer_ = logomanpac::kReadySecs;
        round_elapsed_ = 0.0f;
        wave_index_ = 0;
        wave_timer_ = logomanpac::kScatterChase[0];
        ghost_chain_ = 0;
        write_maze_state();
    }
    ...
    void write_maze_state() {
        ...
        kg.setProperty(maze_entity_, "round_phase", logomanpac::phase_name(phase_));
    ...
    void tick(float dt) {
        switch (phase_) {
            case logomanpac::Phase::READY:
                phase_timer_ -= dt;
                if (phase_timer_ <= 0.0f) {
                    phase_ = logomanpac::Phase::PLAYING;
                    write_maze_state();
                }
                return;
            case logomanpac::Phase::DYING:
                phase_timer_ -= dt;
                if (phase_timer_ <= 0.0f) {
                    if (lives_ > 0) reset_round(/*full_restart=*/false);
                    else { phase_ = logomanpac::Phase::GAME_OVER; write_maze_state(); }
                }
                return;
            case logomanpac::Phase::LEVEL_CLEARED:
            case logomanpac::Phase::GAME_OVER:
                return;
            case logomanpac::Phase::PLAYING:
                break;
        }

        round_elapsed_ += dt;
        advance_waves(dt);
        logomanpac::step_walker(avatar_, board_, dt, /*ghost=*/false);
        eat_here();
    ...
    void advance_waves(float dt) {
        ...
        ++wave_index_;
        wave_timer_ = logomanpac::kScatterChase[wave_index_];
        ...
        for (auto& gh : ghosts_) {
            if (gh.mood == logomanpac::Mood::FRIGHTENED ||
                gh.mood == logomanpac::Mood::EATEN) continue;
            gh.mood = wave_is_chase() ? logomanpac::Mood::CHASE
                                      : logomanpac::Mood::SCATTER;
            gh.walker.desired = logomanpac::opposite(gh.walker.dir);
        }
    }
    ...
    void eat_here() {
        const int c = board_.wrap_col(logomanpac::cell_of(avatar_.col));
        const int r = logomanpac::cell_of(avatar_.row);
        ...
        score_ += slot.energizer ? logomanpac::kEnergizerPoints : logomanpac::kPelletPoints;
        ...
        if (slot.energizer) {
            ghost_chain_ = 0;
            for (auto& gh : ghosts_) {
                if (gh.mood == logomanpac::Mood::EATEN) continue;
                gh.mood = logomanpac::Mood::FRIGHTENED;
                gh.frightened_left = logomanpac::kFrightenedSecs;
                gh.walker.desired = logomanpac::opposite(gh.walker.dir);
            }
        }
        ...
        if (pellets_remaining_ <= 0) {
            phase_ = logomanpac::Phase::LEVEL_CLEARED;
            write_maze_state();
        }
    }
    ...
    void step_ghost(logomanpac::GhostSlot& gh, float dt) {
        ...
        if (logomanpac::at_cell_centre(gh.walker, 0.12f)) {
            const int c = board_.wrap_col(logomanpac::cell_of(gh.walker.col));
            const int r = logomanpac::cell_of(gh.walker.row);
            logomanpac::Dir next;
            if (gh.mood == logomanpac::Mood::FRIGHTENED)
                next = logomanpac::choose_random_dir(board_, c, r, gh.walker.dir, rng_);
            else
                next = logomanpac::choose_ghost_dir(board_, c, r, gh.walker.dir, tc, tr);
            if (next != logomanpac::Dir::NONE && next != gh.walker.dir) {
                gh.walker.desired = next;
            }
        }

        logomanpac::step_walker(gh.walker, board_, dt, /*ghost=*/true);

        // Eyes reaching the house become a ghost again.
        if (gh.mood == logomanpac::Mood::EATEN) {
            const float dc = gh.walker.col - logomanpac::kHouseCentreCol;
            const float dr = gh.walker.row - logomanpac::kHouseCentreRow;
            if (dc * dc + dr * dr < 0.5f) {
                gh.mood = wave_is_chase() ? logomanpac::Mood::CHASE
                                          : logomanpac::Mood::SCATTER;
                gh.in_house = true;
        ...
        } else if (gh.in_house && gh.walker.row <= logomanpac::kHouseExitRow + 0.05f) {
            gh.in_house = false;
        }
    }

    float ghost_speed(const logomanpac::GhostSlot& gh) const {
        switch (gh.mood) {
            case logomanpac::Mood::FRIGHTENED: return logomanpac::kFrightenedSpeed;
            case logomanpac::Mood::EATEN:      return logomanpac::kEyesSpeed;
            default:
                // Held in the house: shuffle, do not sprint.
                if (gh.in_house && round_elapsed_ < gh.release_at)
                    return logomanpac::kGhostSpeed * 0.45f;
                return logomanpac::kGhostSpeed;
        }
    }

    void pick_target(const logomanpac::GhostSlot& gh, int& tc, int& tr) const {
        if (gh.mood == logomanpac::Mood::EATEN) {
            tc = logomanpac::kHouseCentreCol; tr = logomanpac::kHouseCentreRow;
            return;
        }
        if (gh.in_house) {
            ...
            if (round_elapsed_ < gh.release_at) {
                ...
                tc = logomanpac::kHouseExitCol; tr = logomanpac::kHouseExitRow;
            }
            return;
        }
        if (gh.mood == logomanpac::Mood::SCATTER ||
            gh.mood == logomanpac::Mood::FRIGHTENED) {
            tc = gh.scatter_col; tr = gh.scatter_row;
            return;
        }

        const int ac = board_.wrap_col(logomanpac::cell_of(avatar_.col));
        const int ar = logomanpac::cell_of(avatar_.row);
        int adc = 0, adr = 0;
        logomanpac::dir_delta(avatar_.dir, adc, adr);
        ...
        if (n == "BLINKY") {
            ...
            const int bc = board_.wrap_col(logomanpac::cell_of(ghosts_[0].walker.col));
            const int br = logomanpac::cell_of(ghosts_[0].walker.row);
    ...
    void resolve_contacts() {
        for (auto& gh : ghosts_) {
            ...
            if (dc * dc + dr * dr > logomanpac::kCatchDistance * logomanpac::kCatchDistance)
                continue;
            ...
            if (gh.mood == logomanpac::Mood::FRIGHTENED) {
                ...
                ++ghosts_eaten_total_;
                gh.mood = logomanpac::Mood::EATEN;
                gh.frightened_left = 0.0f;
                write_maze_state();
            } else if (gh.mood != logomanpac::Mood::EATEN) {
                --lives_;
                ++deaths_total_;
                phase_ = logomanpac::Phase::DYING;
                phase_timer_ = logomanpac::kDyingSecs;
                avatar_.dir = logomanpac::Dir::NONE;
                avatar_.desired = logomanpac::Dir::NONE;
                write_maze_state();
                return;
    ...
    void write_actor_state() {
        ...
        kg.setProperty(avatar_entity_, "lane_direction", logomanpac::dir_name(avatar_.dir));
        kg.setProperty(avatar_entity_, "desired_direction",
                       logomanpac::dir_name(avatar_.desired));
        for (const auto& gh : ghosts_) {
            ...
            kg.setProperty(gh.entity, "lane_direction",
                           logomanpac::dir_name(gh.walker.dir));
            kg.setProperty(gh.entity, "ghost_mood", logomanpac::mood_name(gh.mood));
            int tc = 0, tr = 0;
            pick_target(gh, tc, tr);
    ...
    void sync_particles() {
        ...
        set_entity_size(view, ps, avatar_entity_,
                        avatar_.dir == logomanpac::Dir::NONE ? 0.92f : chomp);
    ...
    static logomanpac::Board& g_board() {
        static logomanpac::Board b;
        return b;
    }
    ...
    void update_hud() {
        ...
        const char* banner =
            phase_ == logomanpac::Phase::READY         ? "READY!" :
            phase_ == logomanpac::Phase::DYING         ? "CAUGHT" :
            phase_ == logomanpac::Phase::LEVEL_CLEARED ? "LEVEL CLEARED" :
            phase_ == logomanpac::Phase::GAME_OVER     ? "GAME OVER - R TO RESTART" : "";
...
private:
    ...
    logomanpac::Board& board_ = g_board();
    ...
    logomanpac::Walker avatar_;
    std::vector<logomanpac::GhostSlot> ghosts_;
    std::vector<logomanpac::PelletSlot> pellets_;
    std::unordered_map<uint32_t, int> pellet_index_;

    logomanpac::Phase phase_ = logomanpac::Phase::READY;
    ...
    int score_ = 0;
    int high_score_ = 0;
    int lives_ = logomanpac::kStartingLives;
