...
struct Character : public LivingEntity {
    ...
    /// Soc, position 6 of the UPP. Values 10+ map to noble titles [book1/character-creation.md "Social Standing and Noble Titles"].
    std::optional<int32_t> social_standing = std::nullopt;
    /// Psi, the seventh characteristic; only exists once psionics are learned, never rolled at creation without the Referee [book1/character-creation.md "Psionic Strength, the Seventh Characteristic"]. Absent means untested.
    std::optional<int32_t> psionic_strength = std::nullopt;
    /// Pseudo-hex summary, "687B9C" or "687B9C-4" with Psi [book1/character-creation.md "The Universal Persona Profile"]. DERIVED from the seven scores by rules/ehex; stored for the referee and the journal to read.
    std::optional<std::string> upp = std::nullopt;
    /// Chargen starts at 18 [book1/character-creation.md].
    std::optional<int32_t> age_years = std::nullopt;
    /// Completed terms of service, across a whole life.
    std::optional<int32_t> terms_served = std::nullopt;
    /// Rank in the CURRENT career, 0 until a commission. Reset on entering a new one: rank belongs to the service, not the person. In the graph because rules read it - "characters of rank O5 or O6 gain +1 on Material Benefit rolls" is a modifier that asks the character a question, and it can only ask what the graph holds.
    std::optional<int32_t> rank = std::nullopt;
    /// Completed terms in the CURRENT career, kept apart from terms_served because the book pays benefits per career and counts aging by a whole life. Also what "who have retired" is measured against: "A character who has served 5 or more terms in a single service receives a yearly retirement pension".
    std::optional<int32_t> terms_in_career = std::nullopt;
    /// How many careers the character had already entered BEFORE the one they are being asked about. Written just before a Qualification throw, because that is the only question that asks it: "You suffer a DM-2 to qualification rolls for each previous career you have entered." Derived rather than counted twice - the career being attempted has already been recorded by then, and is not previous to itself.
    std::optional<int32_t> previous_careers = std::nullopt;
...
/// One four-year term of service actually lived by a Voyager during character generation [book1/character-creation.md "Terms of Service"]. The KG accumulates these; the referee narrates them.
struct ServiceTerm : public Event {
    /// 1-based index of the term in the Voyager's life.
    std::optional<int32_t> term_number = std::nullopt;
    /// Career name this term was served in.
    std::optional<std::string> term_career = std::nullopt;
    /// The survival roll's outcome, engine-rolled.
