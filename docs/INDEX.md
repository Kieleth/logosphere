...
- **[examples/voyager](../examples/voyager/README.md)** — character creation up to the first door, and no further: six characteristics rolled from the graph, a referee that narrows twenty-four careers to the few that fit this person, and nothing the book fixes written in C++. A test adds a seventh characteristic to the graph at run time and fails if the screen does not grow. Plan and what it closes in [VOYAGER_PLAN.md](todo_plans/VOYAGER_PLAN.md)
