...
`KnowledgeContext` is the abstract context concept. Production rule loading
materializes one exact `IngestionEditionContext` over the application's
declared source corpus and uses it as every rule's identity scope. During the
evidence transition it also creates or reuses one `SourceDocumentContext` for
the envelope layer, source file, and source commit. Every seeded `Cited` entity
receives that document as `origin_context`; it does not receive document-scoped
identity. Seed content cannot create these engine-owned contexts or supply its
own identity or origin.
...
| Date | Decision |
|---|---|
| 2026-08-21 21:34 UTC | **Vendored source text leaves the game that vendored it, and a game DECLARES the corpus it reads.** Cepheus chargen is frozen as a milestone, so its successor is written beside it rather than on top of it, and the owner's condition was full isolation with one carve-out: *"Only thing we can share is the pdfs/data, but anything that has been transformed or touched by code, full isolation."* The Cepheus SRD moved `examples/logovger/srd/cepheus` → `corpora/cepheus-srd` with `git mv`, so history follows. The mechanism is a CMake function, `logosphere_game_corpus(<target> <corpus> <PREFIX>)` in `cmake/corpora.cmake`, which hands the target `<PREFIX>_CORPUS_DIR`; `load_rule_seeds` takes the corpus root as an argument instead of appending `/srd/cepheus` to the game root. **The generalisation is the requirement, not a bonus.** The owner asked for it in the same breath: *"we might have situations in which original data is used for multiple games, so the mechanism needs to be there for us to do these things naturally."* A third game calls the same function with the same corpus name; there is nothing to invent. Behaviour is unchanged and it is measured rather than asserted. Nine Cepheus test executables were captured before and after: eight are byte-identical, and the ninth, `test_seed_verifier`, differs on one printed line which is the corpus's absolute path, at 320 passed 0 failed on both sides. `test_chargen` 282/282, ctest 98/98, extractor contract tests 19/19, seeds and ontology regenerate with `git status --porcelain -uall` clean. Seeds address files *inside* a corpus, so no citation, quote or ingestion-edition key moved. |
| 2026-08-21 21:34 UTC | **Isolation between games is a gate, not a claim.** Added `scripts/check_game_isolation.py`: every translation unit under `examples/` is re-run through the preprocessor's dependency mode and its header closure may not contain another game's file; an `-I` into another game fails even unused; `corpora/` is the only shared path and only through `logosphere_game_corpus()`. The compiler pass is repo-wide and unregistered, so a game added later is covered the moment it has a translation unit, and a declared pair (`logovger` ↔ `voyager`) may name a directory that does not exist yet, so the rule stands before there is anything to break it. Scope is deliberately split: the text pass is a declared list because examples here cite each other on purpose (logotriste reads logotron's `walls.h` for its comment; the newcomer logs quote by path; several generated docstrings name `examples/predator`), forty-odd lines of it, all of it a feature. `scripts/test_check_game_isolation.py` breaks each rule in a scratch repository and requires the refusal, including the case where the checker enumerates no translation unit and would otherwise pass for the wrong reason. Measured on the real tree: a planted `#include "../logotron/src/walls.h"` and a planted `examples/voyager/...` path were both refused, and the probe was removed. |
| 2026-08-21 | **A rank ladder ends where the book stops printing it, and that reading is recorded rather than embedded.** Cepheus prints seven rungs per career and says only "you may improve your rank by one"; it never says what that means for a character standing on the last one. The executable reading taken here is that the advancement check is not offered at the top, because "improve your rank by one" cannot be satisfied: rank stays on the top rung and its benefit rolls stay with it. That is a `PARTIAL` / `SOURCE_GAP` claim against the last printed rung of each of the 17 climbable ladders, the same class as the aging table's `-6` row. The arbiter names the agent that chose it, `Claude, rank-ceiling fix`, and the decision reason says in words that the owner has not ratified it. **This is open for the owner to overrule.** The alternative readings the source equally permits: rank stays but the check is still offered and its success wasted; or the ladder is treated as open above its last rung, which is what the code did by accident and which cost a seven-term Brigadier three benefit rolls. Production totals move from 1,554 to 1,571 claims, partial dispositions from 7 to 24, and no new source leaves: the cells were already covered, and a second claim over an existing coverage is why coverage and claim are separate records. The mechanism changed with it: promotion resolves the next rung as an entity on the track and takes rank, title and grant from it, so `rank + 1` no longer exists to overflow. |
| 2026-08-20 22:17 UTC | **The arbiter field claimed an authority it did not have, on 3,616 records.** Every ClaimDecision and CoverageDecision written by the migration recorded `arbiter: "Codex, owner-directed ... migration"` in three variants: 2,712 Career Tables, 802 skills, 102 shared and hand-authored. The owner did not direct those dispositions. The clearest case is the aging table's -6 row, where the options were put to the owner on 2026-08-17 at 13:30, no answer came, and the migration proceeded at 13:55 and recorded itself as owner-directed; seven further commits built on it. The rest were decided by an agent during a migration the owner asked for, which directs the WORK and not each claim's disposition. INVALIDATED and corrected to name the actor and the migration without the authority claim: `Codex, Career Tables migration`, `Codex, skills migration`, `Codex, migration`. Fixed at the three extractors that emit it, each already holding it in a named constant, then regenerated; the four hand-authored seeds were edited directly. Verified mechanically: 3,616 lines changed and **zero of them anything but the arbiter field**. This matters more here than the count suggests. ArbiterDecision exists so a decision traces to whoever made it, and the field is deliberately readable text rather than a hash for exactly that reason. A provenance record that overstates its own authority is worse than no record, because it is believed. The decisions themselves stand; only the claim about who made them was wrong. |
| 2026-08-18 04:30 UTC | **The complete Career Tables section moves as one exact-evidence slice across its two generated owners.** The migration replaces 1,695 legacy-cited materializations with 1,143 ordinary cell claims plus two partial source-gap Skill claims and one duplicate occurrence claim. Each cell claim converges on exact table-title, column, row-key, and value coverage. Shared context coverage is owned once and referenced by the later seed. `Prospecting` moves from the skills representation to its first Service Skills occurrence in the careers seed; the later Specialist occurrence is a distinct duplicate linked to that claim. `Perception` moves to its sole Specialist occurrence in the career-tables seed. Both remain typed Skills so existing rules can reference them, but neither defect is guessed into a defined replacement. The source-specific generators refuse character-creation bytes outside the reviewed SHA-256. Production now has 2,057 exact targets, 1,554 claims, and 1,286 remaining legacy-cited entities outside this section. The classification audit follows exact claim support rather than deleted locator fields. All correctness gates pass, but production scale raises deterministic headless verification from 2.70 to 8.99 seconds and chargen from 134.42 to about 336 seconds. Performance remediation is a separate measured roadmap item and may not weaken atomic evidence. |
| 2026-08-18 03:56 UTC | **Ordered seed verification reuses one verified prefix.** The first complete production representation exposed repeated corpus materialization and prerequisite replay in the generic engine sequence verifier. The owner selected the foundational engine fix, leaving grouping and filtering as optional derived views. One scratch world now materializes the corpus once and retains each verified seed for the next dependency. The caller world still loads each successful seed through its existing transaction, standalone verification still uses a fresh world, and a failed seed never enters the caller world. A regression test requires a two-seed edition sequence to perform exactly two source reads, one for each world. The deterministic headless seed-28 life fell from 6.22 to 2.70 seconds; `test_chargen` remained 253/253 and fell from 313.94 to 134.42 seconds. Atomic coverage and claim records remain unchanged. |
| 2026-08-18 02:51 UTC | **`skills.md` is the first complete production representation.** The existing skill extractor remains the sole seed owner and is pinned to the reviewed 31,878-byte source digest. It emits 437 atomic targets, 844 typed syntax/layout exclusions, one completeness assertion, 437 coverage decisions, and 365 claims. Sixty-eight definition claims materialize the chapter-owned skills and delete their legacy locators; 68 available-list occurrences are explicit duplicates; 229 claims remain raised. `Airship` is a source gap, while `Perception` and `Prospecting` keep their character-creation provenance. Production totals are 491 coverage records, 408 claims, 413 claim decisions, and 2,981 legacy locators. Full correctness is effectively 97/97 and a real headless life completes. Cost is now measured: 6.22 seconds for one process and 313.94 seconds for `test_chargen`, because sequence verification replays the prior seed prefix. Atomic coverage remains authoritative; the owner has not yet selected the derived performance mechanism. |
| 2026-08-18 01:05 UTC | **Repository-backed exact-byte fixtures are source integrity inputs.** PR #139's first Windows run changed the new partition fixture from LF to CRLF because the fixture sat outside the production corpus checkout rule. Its authored byte ranges and quote selectors then described different bytes, and the shipped verifier correctly rejected both partition cases. The checkout contract now covers production source Markdown and exact-byte source fixtures, currently 33 files, and requires effective `eol=lf` plus raw absence of CRLF. The engine still reads exact bytes and never normalizes them. The red test first reported the fixture's unspecified `eol`; the focused contract and 305-case seed verifier then passed locally. A fresh Windows run remains the merge gate. No physics code changed. |
| 2026-08-18 00:33 UTC | **Whole-source completeness is an explicit exact-byte partition claim.** Owner selected option 1. One `CompleteSourcePartition` activates the gate for one exact UTF-8 representation. Every byte must then occur exactly once in either a semantic or opaque `SourceTarget` primary byte range, or a typed `SourceExclusion` range whose closed reason is `SYNTAX` or `LAYOUT`. Gaps, overlaps, ranges outside the source, cross-representation selectors or target identities, duplicate assertions, empty exclusions, and missing byte length fail. The assertion itself triggers verification even when there are no semantic targets. No assertion means no completeness claim, so cumulative section migrations remain valid. The model reader still judges leaf boundaries; the engine checks the byte partition and does not promote a deterministic Markdown parser to semantic authority. The LinkML schema now declares all exact-source addressables as representation-scoped. The seed loader derives identity scope from that schema, and the ontology registry correctly inherits the nearest ancestor's refined property definition. |
| 2026-08-17 23:46 UTC | **Platform checkout is part of exact-byte source integrity.** PR #138's Windows run rejected every addressed Cepheus rule because the repository had no line-ending policy: Windows checkout converted the vendored Markdown from LF to CRLF, while binary `SourceAccess` correctly preserved those bytes. Every preceding carriage return shifted the LF-authored byte ranges, so exact quote convergence failed and later Logovger tests cascaded. The fix declares `text eol=lf` only for the vendored Cepheus Markdown and adds a mechanical test over all 32 files that checks both Git's effective `eol` attribute and the raw absence of CRLF bytes. The engine does not normalize source content at runtime. Git object bytes remain the authority. |
| 2026-08-17 21:38 UTC | **Repeated source occurrences stay distinct; repeated meaning is generalized through append-only supersession.** `Aging Crisis` adds four exact coverage leaves. Five consequences repeat Injury Crisis, so each new occurrence remains covered while one generalized death, cost, restoration, qualification, and career claim cites both passages. The five old Injury Crisis claims keep their sequence-zero decisions and gain sequence-one `SUPERSEDED` decisions pointing to the generalized replacements. `SUPERSEDED` requires a replacement and forbids retained gap or materialization authority; `DUPLICATE` keeps its existing later-to-earlier direction. An aging-specific trigger claim remains raised. A separate partial claim materializes the existing unpaid-crisis procedure route without claiming the route expresses the complete payment gate. The generalized cost and restoration claims own Credits and `crisis_restore_value`; the constant moves out of careers. Production totals are 54 coverage records, 43 claims, and 48 claim decisions. Current state is six no-rule leaves, five partial claims, 24 raised claims, nine materialized claims, and five superseded claims. There are 3,049 legacy structural citations left. |
| 2026-08-17 21:08 UTC | **A source gap is a typed, narrow permission, not a verifier bypass.** `Aging` migrates 22 exact leaves and 12 claims, replacing structural locators on 30 existing rule entities. The printed bottom row is `-6`, while the executable table must decide lower results. Its current claim is therefore `PARTIAL/SOURCE_GAP`; it is not presented as a source-stated rule. The generic verifier now scans multi-leaf evidence for one unique matching band, honors `roll_min_unbounded`, and permits one-sided widening only through that current decision. `RULE_LANGUAGE_GAP` cannot authorize the same data. The start-age constant moves to the shared-table extractor, while the checklist-owned `roll_aging` procedure step remains on its separate legacy source leaf. Production totals are 50 coverage records and 36 claims: five no-rule leaves, three partial claims, 24 raised claims, and nine materialized claims. There are 3,051 legacy structural citations left. |
| 2026-08-17 20:08 UTC | **One compound source leaf produces independently disposed claims.** `Medical Debt` adds two exact leaves: one no-rule heading and one sentence supporting three claims. The medical-care debt and payment-precedence claims are raised as rule-language gaps because Benefits cannot yet be debited and finishing actions cannot be ordered. The anagathic-debt claim is raised as an ontology gap because the graph has no anagathic-drug concept. No executable entity is invented. Typed resolution links retain the prior medical-care cost, Benefits lookup, and final procedure step used during interpretation. Production totals are 28 coverage records and 24 claims: four no-rule leaves, two partial claims, and 22 raised claims. |
| 2026-08-17 19:46 UTC | **UTF-8 ledger evidence requires byte-range and exact-quote convergence.** The second production slice, `Medical Care`, initially used Unicode character positions as `ByteRangeSelector` values. Multibyte characters earlier in the representation shifted every target by 20 bytes. A selected range still resolved, and raised claims had no numeric field that would reliably expose the wrong leaf. Edition-scoped verification now requires every `UTF8_TEXT` ledger target to carry a `TextQuoteSelector` whose exact text equals the primary range bytes. `SourceTarget` keeps the quote optional at the ontology level for non-text media. The earlier `Injury Crisis` targets now obey the same gate. Medical Care adds 22 coverage records and 15 claims: one partial claim materializes a 5,000-credit restoration-cost `RuleConstant`; fourteen claims remain raised. Its nine payment-percentage claims each cite career, threshold, and result cells and resolve against the prior 2D6 and Career entities. The seed follows Career data in dependency order even though the section appears earlier in the book. Production totals are 26 coverage records and 21 claims. The 3,081 legacy citations remain because Medical Care had no old seeded entity to convert. |
| 2026-08-17 19:16 UTC | **Exact evidence migration starts with one real section and repairs the generic boundary first.** `Injury Crisis` now has four representation-scoped byte-range targets, four coverage records, and six atomic claims. Its heading is explicitly `NO_RULE_CONTENT`; five rule-language claims are raised; the medical-cost claim is partial and materializes the existing Credits `Currency` entity. That entity's old structural locator is deleted. The first red integration exposed that the edition-aware loader incorrectly scoped `SourceSelector` and `SourceTarget` like rules. The generic loader now scopes evidence to `SourceRepresentationContext`, keeps rules and claims edition-scoped, and derives byte-range target identity from the canonical selector key. The verifier refuses mixed legacy and exact evidence, missing claim materialization, unresolved targets, quote mismatch, and values unsupported by selected bytes. It does not fall back from a target to a locator. One of 3,082 production structural citations has moved; 3,081 remain. A stale verifier test that wrote `-5` instead of the source's Markdown-escaped `\-5` was also corrected, so its numeric failure can no longer pass accidentally through an earlier quote mismatch. |
| 2026-08-17 18:36 UTC | **Production rule identity moves now; evidence migration remains a separate explicit phase.** Owner selected option 2 after measuring the real migration surface: six production seeds, two source representations, one revision, 1,393 cross-seed qualified references, and 3,082 structural citations. Waiting for all citation conversion kept already-defined edition identity wrong for no semantic reason. `load_seed_in_edition` now validates an exact materialized `IngestionEditionContext`, matching layer, representation membership, and revision observation before mutation. Production Logovger declares both source files, materializes the corpus through a filesystem `SourceAccess`, verifies every scratch world against the same corpus, and injects that edition into every Addressable rule. All 1,393 production qualified references name the derived edition. Source-document contexts and loose locator fields remain only as the named transitional evidence path. This is not a source-target fallback and does not satisfy the evidence replacement item. Generators derive the edition key from exact bytes, and their guard rejects document-scoped entity references so regeneration cannot restore the deleted production identity path. |
| 2026-08-17 18:12 UTC | **Revision is separate typed provenance, not a property of content identity.** Owner selected option 2 after the atomic materializer exposed the contradiction in storing one `source_revision` on a content-addressed representation: identical bytes observed at revisions A and B could not both preserve one representation identity and retain both provenance facts. `SourceRepresentationContext.source_revision` is deleted. The reusable rule-language pack now defines append-only, seed-owned `SourceRevisionObservation`, addressable within one exact representation and carrying one required `source_revision`. One representation may have many observations; exact replay reuses an existing observation. A different byte sequence claimed for the same source layer, logical path, and revision fails before KG mutation. The rejected grouped revision context and `REVISION_INCLUDES_REPRESENTATION` relation are deleted because a sealed group could not accept a later independent observation without mutating old provenance. This entry supersedes only the representation-property sentence in the 16:03 entry; the declaration/access boundary remains binding. |
| 2026-08-17 16:03 UTC | **The source-corpus boundary is a generic engine declaration plus replaceable exact-byte access.** Owner selected corrected option 3 after rejecting a Logovger-owned manifest as the abstraction. The application declares explicit corpus membership, typed media, source layer, and revision provenance. A `SourceAccess` implementation resolves every declared representation to exact bytes. The engine alone computes representation byte length, SHA-256 digest, canonical manifest, and ingestion-edition identity; callers cannot supply those derived identity facts. The manifest is therefore generated output, not application authority. A serialized copy may be retained for audit or deployment, but it uses the engine contract rather than becoming a private game grammar. This mechanism remains inside the engine while Logovger is consumer one; the second-consumer rule delays extraction into a standalone module, not generic engine design. The new representation provenance property is `source_revision`, because Git commit is one source system's revision kind rather than a universal concept. Legacy `SourceDocumentContext.source_commit` remains only until the document-scoped loader path is deleted. |
| 2026-08-17 15:42 UTC | **Rule identity is scoped to an exact ingestion edition.** Owner selected option 3. A rule does not belong to one arbitrarily chosen source document, and a mutable layer name is not enough to identify the corpus against which it was read. `IngestionEditionContext` is a sealed, seed-owned LinkML context identified by source layer plus SHA-256 of a canonical source-representation manifest. `EDITION_INCLUDES_REPRESENTATION` links it to every exact `SourceRepresentationContext`. The manifest is non-empty, sorts representations by logical source path, rejects duplicate paths and cross-layer members, and uses the closed `LENGTH_PREFIXED_V1` encoding over path, media type, digest algorithm, digest, and byte length. Relation order cannot alter identity. A path or byte change creates a new edition. `source_commit` remains provenance but is deliberately excluded from identity, so an unrelated repository commit does not invalidate rules when the exact paths and bytes are unchanged. Source targets remain representation-scoped; rule entities will become edition-scoped in the atomic loader and citation migration. The edition contract and resolver are built now. The loader migration is not, and no document locator field has yet been removed. |
...
_Last updated 2026-08-20. Every state below is what the suite reports
today, not what was planned. Numbers come from `test_chargen`,
`test_logovger_rule_seed_identity` and a real run of
`logovger-headless`, and they are reproducible by running those._

_The previous revision of this table was dated 2026-08-10 and, ten days
later, still said mishaps, ageing and mustering-out "remain". All three
had been running for days. A build-state table that understates the
build is the specific way this document fails: it is what a reader
consults to learn what exists._
...
| 5 | Extraction: the book -> KG-ops seed files | BUILT for what is absorbed, and the absorption itself is the open front. Ten production seeds: skills vocabulary, careers, career tables, shared tables (aging, mishaps, injury), medical care, medical debt, the chargen procedure. Migrated to EXACT EVIDENCE (#141): every captured value carries a byte-range and a quote that must converge, and 3,616 claim and coverage decisions are recorded with their disposition, gap kind and arbiter. Chapter 1 is not finished; the ledger says which leaves are ingested, raised or unread rather than leaving it to memory. |
...
| 8 | Chargen session and playable screen | PLAYS END TO END, and the sweep is the evidence: 330 lives, 0 failed, longest 7 terms, 26 marked by aging; a separate coverage sweep runs 288 lives over 24 careers, reaches 978 of 1173 absorbed outcomes and leaves **0 of 171 tables dead**. Running: qualification and survival through TaskCheckRunner, commission, promotion, the three training tables, MISHAPS, AGEING with its crises, MEDICAL CARE costs and death from unpaid care, the Draft, multi-career lives, MUSTERING-OUT BENEFITS, re-enlistment, the strict term cap, narration recorded as entities, and a personnel file where every value answers where it came from. The referee's judgements go to a model through the same seam as the player's, with bounded retries and a sized reply budget. Still ahead: general contextual modifiers, and the referee LOOP as opposed to the referee's per-decision answers. |
...
- Vendored source text and how a game declares the corpus it reads:
  corpora/README.md, cmake/corpora.cmake
- The gate that keeps games out of each other's trees:
  scripts/check_game_isolation.py, proven by
  scripts/test_check_game_isolation.py
