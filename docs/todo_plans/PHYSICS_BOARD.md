...
| R7 | **`is_quat_driven`: the fold you asked for, against a finding you never saw** | You instructed it be FOLDED into `solver_mode`, "since it's the same logic/essence". The study then found the flag does two jobs: representation (which orientation field is truth, six correct sites) and authority (paired with `owner`, seven wrong sites). The authority half folds exactly as you said. The representation half has nowhere to fold to, because `solver_mode` does not encode which quaternion is truth. Options: fold the authority half and rename the representation half; fold both by adding an orientation-source field elsewhere; or something you see that we do not. **The board recorded "renamed not folded" for weeks without recording your instruction at all.** Blocks D1's slices. |
