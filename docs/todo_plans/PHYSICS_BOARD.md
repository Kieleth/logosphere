...
## STRATA SEAM HOLES — the creation door refuses a tile beside a crept tile (G-99, found 2026-09-19) — NEEDS DESIGN, owner ruling owed

Found by G-98's cost sheet: the one verdict `DRIVE_WAKE=1` moved was
`test_leg_geometry_choreography`, red with the 0.550 m fingerprint of the
2026-09-02 collapse. Not the wake: the bisect by wake count
(`DRIVE_WAKE_UNTIL=k`) stays green through the tenth wake, and the door's
log (`CREATION_DOOR_VERBOSE=1`) names the cause. When chunk (-5,-1) is
born (f4990) the creation door refuses 14 of its tile births in the column
x = -82 (y -18 .. -2, all three layers) because the eastern tiles of chunk
(-4,-1) had crept west by 2 to 13 mm and tilted (P15 at -78.005, P5 at
-78.013, ROTATED), past SLOP. The lever world's path, 1.2 m south of the
default's, crosses that seam at y = -7.9 into a hole three layers deep and
4 m wide; she free-falls to the turtle from f5570 (the sink grows by g dt^2
per frame; the floor dump at that frame shows no body west of x = -80
within 3 m of her, where the default world shows the stack at (-82, -6)).
The default world refuses 13 births of the same class at seams off her
path (x = -82 at y -30 .. -22; x = -102 at y 14 .. 18): its green is the
drift lottery, and tiles she never touched had crept 1 to 4 mm.

Two mechanisms meet: a settling three-layer stack on the turtle creeps
(the physics finding: a 2400 kg slab is not where it was born), and the
door is exact by decree (INV-37, no exempt classes). Options for the owner,
in G-99: (a) the stack does not creep (the contact law for a slab at rest
under gravity and a light walker: the deeper mechanism); (b) the generator
births beside what is there (a gap, never a hole); (c) tiles as rails
(refused by INV-1). Recommended: measure (a) first with an isolated stack
(one chunk of three layers on the turtle, no walker, the tiles' drift over
2000 frames), since a stack that creeps on its own is a solver defect the
door merely reports. Instruments: `LEG_CHOREO_ROWS=1` (rows + the floor
dump the frame a sink begins), `CREATION_DOOR_VERBOSE=1`.
...
| 2 | the shape pass keeps its placement hands off the drive children (`shape.entity_translate`, `shape.snap_to_hips`, `shape.angular_integrate`) | G-81 | **LANDED 2026-09-08 behind `INV40_STEP>=2`, default off.** Hands 50,514 -> 12,463; walk 4.838 -> 4.929 m; drive walk 4.860 -> 4.935 m, 0 backward. **Flip blocked on G-84:** the driven idle head walks off at 1.3 m/s once nothing re-places it - its eight riders (hair, ears, eyes) are KINEMATIC, re-placed at head + offset every frame, outside the head's bonded component (the union skips KINEMATIC endpoints), and the inner eyes push it by contact (neck stage drift 0.000 -> 3.838 m; measured with the RAILS_IDLE/RAILS_NECK probe). **The body lines** (owner ask, same day: 'make sure all parts of the body are where they are supposed to be'): every nail within 10 SLOP, every bone within its standing reach. Hands on: body whole, hip and knee nails 14-15 cm open from frame 1-5 (the snap places bones where the nails do not meet). Hands off: leg nails 5-8 cm, the spine-neck nail opening 5 mm/frame during the walk (1.46 m by frame 299) - the inner eyes overlap the head 14-19 mm every frame and hold it back while the chest walks; the gauge alone had hidden both. **RULED (a) 2026-09-08 and landed under the same lever:** the riders are parts of the head (DYNAMIC, ten nails holding their rest relation, the head-snap hand off). Measured: the spine-neck nail holds through the walk (worst nail now right_ankle<>right_knee 0.079 m), body whole, walk 4.952 m, drive walk 4.916 m; the driven idle head stays (neck stage drift 3.838 -> 0.002 m). Regression booked on the neck stage: its yaw settles at 0.184 of 0.393 with eight welded riders on the head against its weak 200/12 scalar drive (two_joints family, INV-13). The flip of step 2 is now an ordinary owner QA ruling. The readings were: (a) the riders are parts of the head, not rails (DYNAMIC, nailed, the head-snap hand deleted; the law's own answer), or (b) the union admits a KINEMATIC body nailed into a dynamic structure. |
| 3 | the velocity and ground hands come off the drive children (`locomotion.velocity_broadcast`, `shape.ground_correct`, `shape.ground_vz_zero`, the writer's gravity model `gravity.apply` / `gravity.on_ground_clamp`, a seventh hand found by the prover, and **the eighth hand, `handle_collision_events` = `shape.obstacle_push`, found untraced by the window's SPACE on 2026-09-09, G-88**) at `INV40_STEP>=3` | G-81, G-88 | **LANDED 2026-09-08 behind the lever.** Hands 17,863 -> 0: **the G-81 line is green.** Walk 4.952 -> 4.941 m / 0 backward; drive walk 4.968 m / 0 backward; body whole; worst nail 0.108 m. Attribution (`INV40_KEEP`): with only the clamp still on, 4.110 m / 11 backward / ankle nail 0.219 m - the clamp zeroed the muscles' downward velocity while nothing carried them; all hands off, the rows carry the limbs. |
| 4 | gravity keyed on the momentum door alone; the `is_quat_driven`/owner exemption skipped at `INV40_STEP >= INV40_STEP_MUSCLES_WEIGH` (schema, 4) | G-82 | **LANDED 2026-09-08 behind the lever.** Box A falls 1.299 m (G-82 green); the muscles weigh: walk 4.941 -> 5.063 m / 0 backward, drive walk 4.951 m / 0 backward, body whole, nails unchanged. **Under `INV40_STEP=4` the prover reads 9 of 10.** The last red is the nails line: G-85, the stance chain between two rails opens at the nails (~10 cm) because the drives cannot yield - **the bounded drive is now on the critical path.** Sweep under the lever next; the flip is one ruling. |
...
**THE LEVER WORLD, measured 2026-09-08 evening (sweep alone under `INV40_STEP=4`: MOLES 52 = 43 standing unaudited + 8 new reds + 1 gone-green).** Attribution by `INV40_KEEP` found the idle collapse (feet 3.8 m up, the hips leaving their spine by a metre) in the ground correction: the harness measured its FEET and moved itself, a loop with delay once no hand moved the feet with it. **G-86 landed** (the harness measures its own height above the ground under its column): idle, full_idle and the four movement cases green under the lever; the walk 4.839 m (prover) / 4.906 m (drive walk). Remaining lever reds: `swap_integrity` needs the rail's spin in the ledger (green with `KINEMATIC_LEDGER=1`: the two levers belong together); the G-85 family (nails ~10 cm: `strata_integrity` 0.31-0.47 m foot-shin, `movement_instrumented` 47 jitter violations, `headless` hips-abdomen +0.6 m in a turn, `pin_gluon_lifecycle` foot 1.57 m off its anchor); `idle_pose_stability` at step 4 short by 1.7 cm on a 0.08 m budget; `neck_yaw` 0.184 (the welded riders vs a 200/12 drive); `animation_layering` GONE GREEN. **Sweep alone under BOTH levers (after G-86): MOLES 47 = 43 standing + 4 new reds**, all four the ledger's: two strafe directions reversed (-133 %, -225 %), walk_forward 32 backward frames, turn_in_place drift 0.264 m, and the face pi off the head. Read: the hips' velocity field was both the controller's command and the ledger the derivation overwrites; **the harness now keeps its command in its own state** (`harness_vx/vy`, lever world) and the riders' nails rotate their offsets. Measured under both levers: strafe 5 of 5, walk_forward 0 backward, turn drift 0.021 m, face 0.000 rad, prover walk 5.052 m, drive walk 4.927 m. **CORRECTION + the teleport door (late 2026-09-08):** the direct runs behind the previous line were `INV40_STEP=4` alone (in zsh an unquoted `$E` does not word-split); the both-lever sweep (`MOLES 44`, new-red 0 by class, but read by verdict) confirms strafe, walk_forward and turn_in_place green and puts `eva_movement` and `face_tracks_head` red under BOTH levers while each lever alone is green. `eva_movement`: the test's reset between cases is an undeclared teleport the ledger read as 412 m/s; `reset_humanoid_position` now declares the jump (rails void history, `forget_body`); 4 of 4 green under both. `face_tracks_head`: pi on every feature under both levers, unread (G-38 suspect). **FINAL SWEEP on the teleport-door binary (both levers): `MOLES 43`, the standing unaudited set. By verdict, six stages are green by default and red under both levers: `face_tracks_head` (pi, unread, G-38 suspect), `movement_instrumented`, `strata_integrity`, `headless`, `pin_gluon_lifecycle` (the G-85 family), `idle_pose_stability` (1.7 cm over a 0.08 m budget). Everything that walks, strafes, turns, idles or moves is green in both worlds. **G-87, THE MIRRORED ATTACHMENT (2026-09-09).** The face stage's pi was read: the solver rotates a nail's offsets with the anticlockwise matrix on the engine's clockwise yaw (five copies; CLAUDE.md's own dismember warning; G-20's carrier), so a nailed eye orbits the mirror of its head, and every rotating nail on a turned rig is mirrored. All five copies now route through one helper, clockwise behind `GLUON_OFFSETS_CW` (A/B, default off). Measured on one binary: the eye orbits with the head (+1.90 vs +1.87), face 3.138 -> 0.223 rad, the look-turn's ankle nail 0.356 -> 0.173 m, default world unchanged, **the walking nails unchanged (0.118 m): G-85 stands, the bounded drive is still the walk's answer.** **GLUON_OFFSETS_CW's cost sheet (sweep alone, default world, lever on): `MOLES 43`, zero new reds, zero verdict changes but one: `test_grass_bends_not_tears` red -> green (a bending blade's bond tore at 2.08x by default; nothing tears clockwise; deterministic).** **OWNER RULINGS owed: (1) the flip of GLUON_OFFSETS_CW - recommended, it restores the documented convention at zero cost and heals a booked red; (2) the two-lever flip INV40_STEP=4 + KINEMATIC_LEDGER=1 against its cost sheet; (3) the bounded drive (G-85).** **FRESH EYES 2026-09-09 (owner's ask):** two of the six lever-world reds are not physics: `test_humanoid_headless` and `test_pin_gluon_lifecycle` never call `physics.update()` (their own headers), so under INV-40 nothing in them moves a muscle and their bones stay where they were born while the rail walks (foot 1.574 m from its plant target; hips-abdomen 0.10 -> 0.70 m, measured under the three levers). They measured the hands, and the hands are gone. The physical cost sheet is FOUR stages: face 0.223 rad, movement_instrumented's jitter, strata_integrity's ankle drift, idle's 1.7 cm. The two harnesses need a solver step or a lever-world waiver (ruling 4). Gaps named, not acted on: the hands line sees traced writers only (a gate is owed), the prover's `attach()` is INV-28's fifth copy, G-87 has no default-world prover, step 0's Eden window is still owed. **THE WINDOW'S SPACE (owner QA 2026-09-09, G-88):** the owner's three windows found two driver defects (lamps stranded by streaming swaps; SPACE re-dropping only the boxes) and then a bad reset: the door left the plant target five metres behind (right ankle nail 0.851 m) and, once released, an UNTRACED hand (`handle_collision_events`: the whole rig shoved by any contact's penetration, velocities zeroed) jolted the rail 26 mm per frame when a foot clipped a seam after the teleport, and the arms swung wider each SPACE. Landed: the door releases the plant (`release_plant`, shared with `set_foot_planting`); the eighth hand traced (`shape.obstacle_push`: 67,600 writes on the muscles per 300 default frames, the largest hand by count) and off the whole rig at step 3; `RAILS_REPLAY=n` is SPACE's headless twin (three replays read the base run: walk 5.000 m, nails 0.118, whole, hands 0). Default world unchanged to the millimetre. Sweep alone after it: MOLES 43, the standing set. **THE ARM SWING (owner's eye, 2026-09-09, G-89):** read with Argus (`RAILS_ARMS=1`): the clip commands 0.35 rad from the first stride, the hands hang 10 cm behind the hips and swing 8 cm, the wrist sleeps 60 of 60 frames, and the shoulder drive turns the light BRIDGE (0.2-0.4 rad per second) while the upper arm turns 0.04-0.08: INV-13's two_joints family, not the sleep (waking the arms moves the hang point in front, not the swing: `RAILS_WAKE`). The onset the owner saw was dated by the window's own rows: at one SPACE the hands woke (asleep 208 accumulated frames, then 0 for the rest of the session), moved from behind the hips to in front, and bobbed. The sleep witness (sleep.rest / sleep.wake records, INV-18) shows why they were asleep: the settle rests all six arm bodies at frame 20, the walk wakes the shoulders (rows) and the elbows (the hips rail's reach, every stride), and the wrists never, because nothing in INV-40's world wakes a driven muscle. The window with the witness named the waker: at frame 1 after a SPACE the wrist's row was dissatisfied. **THE SLIDING FOOT (owner's eye, 2026-09-09, G-90):** 'when they touch there's a bit of sliding that is very noticeable' - `RAILS_FEET=1` measures, per contact episode, a touching foot's slide, peak slide speed, its distance from its anchor and the pin's blend; two suspected sources to separate: the ankle nail's opening (G-85) through the stance, and the pin's blend-in dragging a landed foot toward a target it did not land on. READ 2026-09-09, three worlds on one binary: DEFAULT (shipped since step 0) - the 'planted' foot slides 0.57-0.65 m per stance at the hips' speed, its anchor 28-36 cm away, every foot 5 mm above the floor, swing lift 5 mm: a GLIDE, no foot plants (the rest snap over the pin; before step 0 the FK-stamped stance foot was IK-placed at the plant; no pre-step-0 A/B binary built) - **a default-world regression candidate the sweep is blind to; the Eden window QA owed since 96ff4ba is now urgent.** LEVER WORLD - the landing foot is YANKED 0.21-0.27 m in its first 8 frames at up to 8 m/s onto a plant target computed 22-30 cm ahead of where it landed (the stride target and the clip's reach are different data paths), then holds within 3.5-5 cm (G-85's creep), floating 0.5-2 cm above the floor (its plant z is the foot's z at strike); the swing lifts 4.5 cm. Two born-red lines in the prover: a planted foot does not slide (<= 10 SLOP), a planted foot stands on its support (<= 2 SLOP). **OWNER RULING (the design answer): the plant is a rail DECLARED where the muscle touched - the anchor born at the foot's contact point on its support, never computed from the hips and a stride length.** NEEDS DESIGN: the drives' convergence on a light intermediate bone (the joint block solver branch is the candidate); G-89's law gap (a driven muscle asleep) is OWNER RULING. **THE DECLARED PLANT (2026-09-09, G-90, behind `INV40_STEP=5`, default off):** the anchor is born at the foot's own x,y on the support's top, flat, the stance ankle commanded to hold it (`INV40_KEEP=ankle`); the support finder is one helper shared with the ground correction. Against step 4 on one binary: the landing foot's drag in its first 8 frames 0.21-0.27 m at 8 m/s -> 0.6-1.3 cm at 0.5 m/s (THE YANK IS GONE); default world byte-identical. The honest plant exposed four things, each measured behind its own A/B lever and each an OWNER RULING: **(5) the clip's strike pose** - `create_fk_walk_step` lands the foot at 400 of 600 ms at half the peak flexion and returns the hip to zero at the phase boundary, where the writer plants, so the plant finds the foot 5-9 cm ahead (a third of the half-stride the cadence walks; the hip muscle within 0.1-4 deg of its command: authored, not a sag); `WALK_STRIKE=0.925 WALK_STRIKE_HOLD=1` (0.37 rad = asin(0.325/0.9), held to the boundary) lands 0.326-0.328 ahead and leaves 0.319 behind, ankle nail 0.156 -> 0.094, stance path 7-8 -> 3.6-5.5 cm. RECOMMENDED: the strike pose at the step's end with the hip flexion derived from the stride and the leg (one data path, no yank). **(6) the harness's height through the stance** - a straight 0.9 m leg reaching 0.325 m ahead reaches 0.839 m down, 6.1 cm short of a harness held at its rest height: measured 6.3-6.5 cm above the support at every strike, then the foot descends onto its plant over six frames and from mid-stance stands 2-5 mm from its anchor, flat. The retired hip dip did this job; the honest form is the rail's own trajectory: the harness rides over the planted foot on the leg's length (the compass gait's bob, ~6 cm at this stride). RECOMMENDED. **(7) the first step from standing** - the left foot is planted under the hips at frame 1 and held a whole half-cycle, leaving 0.58 m behind: the worst nail (0.166 m) and the body-whole red come from that one stance in every world. A walker's first step is a half step (start at mid-stance, or a shortened first half-cycle): ruling owed. **(8) the physics rate** - `PHYSICS_TIMESTEP = 1/30` in the time system (the engine's call site says 60 Hz) under a writer, rails and drive targets at 60: every muscle stands still on every second frame while the rails move (the per-frame rows change in pairs). `PHYSICS_HZ=60` doubles the stance path with the step count (end separation unchanged: the path law reads jitter, not a slide); cost: physics twice per frame. Ruling owed. The two G-90 lines stay red in every world until (5)-(8) are ruled; the flip candidate is now four levers: `INV40_STEP=5 KINEMATIC_LEDGER=1 GLUON_OFFSETS_CW=1` (+ the clip's strike once ruled). **RULED '6 and 5' (owner, 2026-09-09) AND LANDED:** step 6 (`INV40_STEP>=6`) the harness rides over the planted foot on the straight stance leg's length (G-91, `shape.harness_ride`); step 7 (`INV40_STEP>=7`) the walk clip's strike at the step's end with its hip flexion asin(half stride / leg reach) = 0.399 rad (G-92). Measured on one binary after the standing-start transient: lands +0.31-0.35 m, leaves -0.30-0.32, the hips bob 7-9 cm per step, the landing foot within 0.3-0.7 cm of its support (right stances 0.7-2.8) and 2-5 mm from mid-stance, touching the floor where it never touched, walk 4.967, default byte-identical. **RULING (7) IS NOW THE WALL:** the standing start plants the left foot under the hips and holds it 0.58 m behind for a full half-cycle, so the ride lowers the hips 28 cm over that stance and lifts them 29 cm in ONE frame at the next plant; the hip and spine nails open 0.29-0.30 m, the second stance lands short (+0.19) and the third long (+0.41) before the gait converges. Options: (a) standing IS mid-stance: the walk starts at phase pi/2 with the stance foot under the hips and the swing leg's clip entered at mid-swing (a lurch of the swing leg over the first frames); (b) the first half-cycle at double rate (a half step in time); (c) the ride's height rate-limited to what the muscles follow (hides the cause). Recommended (a), measured before the flip. Ruling (8) unchanged. Flip candidate: `INV40_STEP=7 KINEMATIC_LEDGER=1 GLUON_OFFSETS_CW=1` after (7). **THE COST OF THE WALK (owner, 2026-09-09: 'argus this for performance, I see these demos slowing down'; G-93):** the cost witness (`RAILS_PERF=1`, `RAILS_WAKES=1`) read three things on one binary. (i) The lever world's baseline is 4.0 ms a frame headless against 1.2 by default and 1.0 with the ledger off: INV-39's 'a rail in reach' wake tests sleepers against the rail's bounding sphere, and a 4 m tile's sphere is 2.8 m, so the walking hips wake 5-10 tiles per 30 frames and keep six awake. **OWNER RULING (9):** the reach test reads the rail's swept box against the sleeper's box (recommended; INV-39's own wording names the sphere). (ii) The slowdown is a wave: at every run's end and after every replay the tile under the stance foot is pressed 9-12 mm into its stratum and tilted by the rail-held foot and its unbounded drives, its seams exceed the 3 mm sleep tolerance, the judge wakes the neighbours, and 20-45 tiles wake across 40 m of floor for 60-150 frames: 10-25 ms a frame headless, 40-80 ms a step in the window. Present at step 4 and in the default world; idle across two teleports costs 0.4 ms (the door and the boxes start nothing). **OWNER RULING (10):** the rail-held foot's push on the floor is bounded by the walker's weight (the harness drains the refused-momentum book, INV-40's own last step; the bounded drive, G-85) - the tolerance and the tile's mass are not the fix. (iii) CLEAN, DONE: the window's `[STALL-FRAME]` / `[PHYSICS-SPIKE]` lines printed the last update's timers on every frame of a hold that ran no update (65.13 ms for 8500 frames whose wall clock read 18 ms); they print only on frames whose update ran. **RULING 10 TAKEN AND RE-PLACED (owner, 2026-09-09: 'a foot moves a stone slab, which is very wrong ... TDD, make it RED, physics first'; G-94):** the law is born red in both drivers, thirteen panel lines: the floor body under either foot moves by no more than SLOP over a stance or a swing (9.5 mm at step 7, 4.0 at step 4, 0.0 by default). The physics tried first: the torque-bounded drive (`INV40_STEP=8`: every muscle's angular rows take the bound a force-bounded bond already had) - landed, measured, NOT the fix (9.4 mm); `MUSCLE_K` at 40x weaker cuts the swing's push to 2 mm and leaves the seams 10-14 mm deep. Placed: no ride and the strike held still moves the floor 5.3 mm; idle 0.0; every move is lateral at the seam Eva crosses near frame 235, under the swing foot's toe-off scrape. THE PUSHER: the hips rail dragging the swing foot through rigid bones into the next tile's edge (the tiles rest 1-2 mm proud of each other). **OWNER RULING (10, re-stated):** (a) the rail refuses what it cannot push into the refused-momentum book and the harness drains it (INV-7, INV-40's own last step: the honest physics; the book's carrier must be read first); (b) the swing foot lifts before the rail drags it (the clip's toe-off pushes the toe down while the foot is on the floor: an authoring change); (c) the seams themselves (the night loop's tile-sticking front: tiles resting proud). Recommended: (a) as the law, (b) alongside, (c) is its own front. **RULED 'a then b' (owner, 2026-09-09) AND LANDED:** step 9 (`INV40_STEP>=9`, G-95) the harness drains the hips rail's book and absorbs its plane part as the walker's momentum; step 10 (`INV40_STEP>=10`, G-96) the toe-off lifts. Measured on one binary at 30 Hz: (a) the floor under a foot 9.4 -> 0.7 mm in the base run (G-94 GREEN, the first lever world to read it) and 3.5-3.6 mm across two replays (red: the rail's 33 mm per-step drag spends an overlap on the slab before the drain acts), no seam past 3 mm, no judge wake, no wave, the update flat at 4.3 ms, the hip and knee nails 0.30 -> 0.06-0.07; the book reads 12.5 N.s a frame in the plane (about 750 N, the stance leg's tension at full extension between two rails) and the harness slows to 0.64 m/s: the walk 3.67-4.10 of 5 m, the two-rails conflict now resolving at the harness instead of the nails or the floor; the belt carries 190-320 N vertically, a third to a half of her weight. **NEW RED:** at `PHYSICS_HZ=60` with a replay the drain is a runaway (the book 26 million N.s, the harness 64,000 m/s, the world detonated; step 8 at 60 Hz with replays is fine): a raw velocity kick per frame is positive feedback through the rail's own rows. (b) the swing's floor contact 69-85 -> 50 frames, the floor still moved 4.3-9.2 mm: the scrape is the drive's turnaround lag at full extension, not the toe's angle. **OWNER RULINGS (11-12):** (11) the drain's form: the harness as a body of finite mass in the plane, solved with the rows (the honest form, the road beyond's first step) vs a damped drain (a controller with a time constant, a number); (12) the straight leg at the boundary (G-91's residual): the ride with the loading response's knee flexion so the leg is never singular at the strike or toe-off, which is also the walk's lost speed. Steps 8-10 stay behind their levers, measured, not recommended for the flip; the flip candidate stays `INV40_STEP=7 KINEMATIC_LEDGER=1 GLUON_OFFSETS_CW=1`. **RULING 13 (2026-09-19, supersedes 11; G-97): WHO MOVES THE PELVIS.** The owner, reading the harness ('moves forward at 1 m/s no matter what... why? this seems not right'): the repo's own contract (docs/ARCHITECTURE.md, kinematic_root.h) says the hips are DERIVED from the pinned foot by reverse FK and vault over it; the code that did it (the post-FK anchor shift) is disabled since 2026-04-17 by a comment and no ruling, because it fought the velocity integration and the walk-speed gauge preferred the belt; INV-40 made the belt honest as a rail, and the concession named was balance, not propulsion. Three readings, one question: (A) the belt propels (today: the commanded velocity integrated on the hips rail; every geometric disagreement lands on what cannot refuse; rulings 12, 7, 11, 8 are its residuals); (B) the stance leg's commanded pose propels, as a rail (the contract as written; no new mechanism; deletes the ride and the stance strut; the pelvis stays infinite, so a caught swing foot still moves a slab); (C) the stance leg's muscles propel, physics (the hips DYNAMIC, an orientation-only hold as the balance muscle, harness_vx / the ride / the drain deleted, the commanded velocity turned into cadence and stride; a caught foot slows the walker by the mass ratio; the unknown is the drives' convergence under the walker's load, INV-13's two_joints family). Ruling 11's damped drain is damping as a patch (engine invariant) and is withdrawn; its finite-mass harness is (C). Ruling 12 becomes clip authoring under (B) or (C). RECOMMENDED: (C) as the principle, gated by G-97's isolated rig first (a driven leg carrying the walker's mass from a nailed foot, station C, born red, no harness rework); if the box follows the command on today's solver, build (C) and skip (B); if the light bone turns and the box stays, the joint block solver is the dependency and (B) is the bridge. Nothing built until the owner rules. **RULED 13 (owner, 2026-09-19: '3 for sure, lets refocuse'): (C).** The pelvis is a body with mass, the stance leg's muscles carry it over the nailed foot, the harness holds it upright and nothing else; INV-40's 'two rails' sentence amended, the skill's 'road beyond' clause retired, 11 withdrawn, 12 and 7 become clip authoring. **G-97 MEASURED THE SAME DAY (station C, the isolated rig: Eva's 36.1 kg on her own shin 0.342 + thigh 0.495 m, a nailed foot, the ankle drive sweeping +-0.40 rad in 600 ms x 3):** asleep after the 60-frame hold the rig ignores its drive entirely (240 frames asleep, the load never moves; both worlds: G-89 irreducible); awake (`RAILS_RIG_WAKE=1`) the shin follows within 0.077 rad and the knee within 0.017, the load reaches 55 % then 33 % of the arc and sits 0.345 m off the path, the ankle nail 0.145 m open, the knee nail 0.102, the load's vx 0.000 on every row (the position repair carries it, the velocity pass does not); 2 kg: 80 %, nails 0.023 m; 10 kg: 66 %. G-85's open nails reproduce with no writer and no rail in motion: the solver's nail chain under a heavy end, not the two rails and not the drive's strength. **THE SEQUENCE UNDER (C), by dependency:** (i) LANDED BEHIND `DRIVE_WAKE=1` (G-98, 2026-09-19), OWNER RULING owed on the flip: a driven muscle never sleeps against its command. The cause was never the judge: the row build's immovable exit (2026-08-12) sits four hundred lines ABOVE the drive's own wake (2026-08-09) and claimed it had run, so a sleeping muscle with a closed nail never read its command. Under the lever the drive's angle is checked before the exit through the one wake form every row uses; `GLUON_WAKE_ANGLE` registered (the 0.1f literal, INV-29). Lever off the rig sleeps 240 of 240 frames under a command 0.8 rad away; on, 0 frames and the chain's numbers (55 / 33 %). Residuals: the wake angle is the muscle's dead-band; a row wake resets the damper's counter and never the judge's, so a muscle moved by position repair alone (no velocity) can re-sleep the frame it is turned and comes back by its nails' strain (f065 / f067 in rig_g98_on.log), a counter-convention question for the owner; the scalar angular path has no wake. COST SHEET BY VERDICT (sweep alone, 2026-09-19): default `MOLES 43 (new-red 0, gone-green 0, unaudited 43)`, `DRIVE_WAKE=1` the same aggregate with ONE verdict moved, `test_leg_geometry_choreography` PASS -> FAIL KNOWN-OPEN: a strata seam hole the lever's path finds (G-99, the STRATA SEAM HOLES front), not the wake; (ii) NEEDS DESIGN: the chain solve - a drive's motion carried through nails to a heavy end (the joint block solver branch; the heavy-on-light stack's cure, G-63 / G-64, is the precedent); station C is its gauge, two lines born red; (iii) then the harness rework (the pelvis DYNAMIC, the orientation-only hold - a nail whose position rows are off, its angular drive to a still rail - harness_vx / the ride / the drain deleted, cadence and stride from the command). **RULING 14 owed:** while (i) and (ii) are worked, does the belt stay (the flip candidate as the measured world) or does the derived rail (B: the contract as written, the disabled 2026-04-17 block behind a step) replace it as the bridge? Recommended: the belt stays; B would be built to be deleted, and (C)'s two dependencies are the work. Instruments: `RAILS_RIG=1`, `RAILS_RIG_WAKE=1`, `RAILS_RIG_LOAD=<kg>`, the window's C key; `Argus::rekey` in the scene's swap callback (the witness lost the rig at frame 6 without it).
...
Two mechanisms, both TDD by test_jammed_sleep H then G. SHIPPED ON: the
merge's precondition (a body beside the surface is not on it) - H green,
the blade study green, the leg choreography 0.044 m, Eden steady 621 ms
(door 751, sleeper 749). OPT-IN (SEAM_FACE_AREA=1): the narrow phase's
face-has-area rule at SLOP - it adds case G's four edge tiles at 250 ms
per Eden steady frame and removes a symptom whose reason is open (G-73:
