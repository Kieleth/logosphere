...
| C10 | **FallenTreeGenerator lifts every log clear of the ground** | `test_collision_bounds_rotation` part [1c], born red 2026-08-21: measured lift +0.2600 / +0.2083 / +0.2875 / +0.2400 m per preset against a SLOP of 0.0010 m. The generator's own comment (`src/worldgen/fallen_tree_generator.cpp:170-177`) states the assumption that broke: "The half-extent is half the segment length", true only under rotation-blind bounds. `create_segment` (`:242-259`) lays the log with `rotation_y = pi/2` and writes the LENGTH into `thickness`, so the oriented world-Z half-extent is half the DIAMETER. | One offset, one line, and the test that proves it is already written and already red. INV-4: a structure is born at rest, and a body hovering above the ground it was placed on is that law with the sign flipped. **This is the only adjudication red that lands in a PR-gating CI lane** (`add_headless_test` -> ctest -> `headless-linux`). |
...
- test_tree_wiggly: G-44 UNMASKED a sustained 0.0294 m/s oscillation in
  ...
  *2026-08-21, R10: the ruling came and it was "delete... and create a new
  battery of tests whenever we start to work on controlled movement, like
  wind." The TEST is gone; THE FRONT IS NOT. Last measured: depth 3 and
  depth 4 both 0.0294 m/s with 30/30 wiggly frames, depth 5 at 0.0000 with
  0/30, under the restored original bands (0.01 m/s, 10 frames). Owner
  hypothesis, in the ledger on feat/physics-tdd-item6: the wiggle may be
  BIRTH ENERGY from creation-time overlaps rather than a solver limit
  cycle — so R8's creation no-overlap door is the experiment that settles
  it, and the replacement battery is what measures it under wind. The
  numbers survive in the retired TEST_AUDIT row and in G-44's notes.*
...
| ~~R8~~ | **RULED 2026-08-21: INV-30 governs. A generator handing the solver an overlap is a BUG, not a policy, and the creation no-overlap door is being built by a separate agent.** The 2026-08-02 acceptance does not survive the later ruling. `test_no_overlap_at_creation` is unchanged and still reports; it gates when the door lands, which belongs with the door. The question as it was put: **May a GENERATOR hand the solver an overlap?** Two owner decisions eleven days apart say opposite things. 2026-08-02, carried only by `test_no_overlap_at_creation`'s own policy block and absent from LEDGER.md: "MINIMAL OVERLAP IS ACCEPTED", because driving it to zero cost the tree, 149 bodies became 59, and the gate moved to `test_foliage_stays_attached`. 2026-08-13, INV-30, active, generators named in the statement: no frame begins with an overlap beyond SLOP, enforced strict-first. Measured today: 2 of 11781 pairs, deepest 0.3426 m (342x SLOP), 0 coincident. Full write-up with both positions verbatim and a demand table: `tests/invariants/TO_INVESTIGATE.md`. | Whether `test_no_overlap_at_creation` gates or reports. If INV-30 wins, the tree case runs under the inventory lever until the generator is fixed. If 2026-08-02 wins, INV-30 needs the generated-foliage carve-out written into its own record instead of living in a test comment. Either way the 2026-08-02 decision should land in LEDGER.md. |
| ~~R9~~ | **RULED AND EXECUTED 2026-08-21: "delete".** `tests/test_sleep_diagnostics.cpp` is gone, registrations removed from `CMakeLists.txt` and `src/unified_test_runner.cpp`, `TEST_AUDIT.jsonl` row KEPT as the retirement record. If sleep needs an instrument again it gets written against extremity speed from the start; G-44 holds that. The question as it was put: **retire `test_sleep_diagnostics`, or rewrite its bands to extremity speed?** The mechanism it diagnoses is gone: frame-gated adaptive damping was eradicated 2026-08-14 by owner decree, and `physics_system_v4.cpp:5048-5067` says the `low_velocity_frames` counter survives because "its only remaining reader is diagnostics" — this file. Its printed advice names constants that do not exist (no `LOW_VEL_THRESHOLD`, no 0.5 m/s gate, no 0.98 damping rate) and would tune the exact mechanism G-44 replaced. Its own numbers make the case: 393/393 asleep, damping band empty throughout, summary printing "damping not helping" about a world at rest. Output is marked as dead-mechanism; nothing deleted. | Nothing. It asserts nothing and exits 0 either way. This is a debt-removal decision. |
| ~~R10~~ | **RULED AND EXECUTED 2026-08-21: "delete... and create a new battery of tests whenever we start to work on controlled movement, like wind."** `tests/test_tree_wiggly.cpp` is gone, registrations removed (including the `run_physics_tests` aggregate), `TEST_AUDIT.jsonl` row KEPT as the retirement record with the last measured numbers as the replacement battery's baseline. **THE FRONT STAYS OPEN** — see the G-44 oak-oscillation entry below; the owner's hypothesis, in the ledger on `feat/physics-tdd-item6`, is that the wiggle may be BIRTH ENERGY from creation-time overlaps rather than a solver limit cycle, which makes R8's creation door the experiment that settles it. `test_ancient_oak` still bounds COM speed and still carries the same flaw. The question as it was put: **do the rest bounds become G-44 extremity speed?** `test_tree_wiggly` and `test_ancient_oak` both bound COM speed (0.01 m/s, 25 mm/s). G-44 ruled that quietness must price both channels in one currency, `sqrt(v^2 + (omega*r)^2)`, so a trunk slow at its centre and turning about its base reads quiet to these checks and is not. `is_at_rest` already prices it that way internally, which is why the tile tests assert both halves. | Nothing today. It would sharpen the two tree bounds and would probably surface more of the same defect the depth-3/4 oaks already show. |
