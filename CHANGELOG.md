...
### Added
- **`DRIVE_WAKE=1`: a driven muscle never sleeps against its command (G-98).** The solver's row build skips a bond whose two endpoints are both immovable (asleep or KINEMATIC) before it reaches the quaternion drive's wake-on-angle, so a driven body that fell asleep with its nail closed ignored its drive target until something else woke it (G-89). Under the lever the drive's angular error to its command is checked before that exit, on the same footing as a bond's length: both endpoints marked dissatisfied, the DYNAMIC ones freed. Default off until owner QA. Station C's third law line proves it (born red, green under the lever). `DRIVE_WAKE_DEBUG=1` prints each pre-exit wake with both bodies' state; `DRIVE_WAKE_UNTIL=k` fires only the first k, for attribution.
- **`GLUON_WAKE_ANGLE` (0.1 rad) registered in `schema/physics.yaml`.** The drive row's wake-on-angle literal is now a constant (INV-29 residual table ratcheted from 3 to 2 `0.1f` sites in the solver TU); the row and the new pre-exit check read the same number.
- **Station C in `test_rails_and_muscles`: a driven leg under the walker's mass (G-97, ruling 13).** A nailed foot, a driven shin and thigh of the humanoid's own bone lengths, and a stone load of her mass; two law lines born red in both drivers and on the panel (the load on the command's path; the load keeping up with the arc). `RAILS_RIG=1` prints the rows, `RAILS_RIG_WAKE=1` and `RAILS_RIG_LOAD=<kg>` are stagings, the window's C key looks at the rig.
- **`Argus::rekey(old_id, new_id)`.** The witness follows a particle-index swap (streaming); a watched body that moved to a new id was lost silently before.
...
- **`INV40_STEP=5`: the plant is declared where the foot is.** At heel strike the stance anchor is born at the foot's own position on the support under it, flat, and the stance ankle's drive target is published beside the hip and knee so the foot is held on its plant (`INV40_KEEP=ankle` keeps the clip's ankle). The landing foot is no longer dragged to a target computed from the hips and a stride length (measured: 0.21-0.27 m at 8 m/s in its first 8 frames -> under 1.3 cm). The plant is traced as `plant.declare`. Default off.
- **`INV40_STEP=6`: the harness rides over the planted foot.** During a stance the pelvis harness's height is prescribed from the straight stance leg's length (the joint hierarchy's own bone lengths) over the horizontal distance to the plant, the compass gait's bob, so a foot declared on its support is reachable at every frame; traced as `shape.harness_ride`. Default off.
- **`INV40_STEP=7`: the walk clip's heel strike derived from the stride.** The generated walk step's strike keyframe holds to the step's end and its hip flexion is `asin(half stride / leg reach)`, so the foot lands where the cadence walks. `WalkStepProfile` gains `hip_flex_strike` and `strike_holds` (defaults keep today's clip). Default off.
...
- **`INV40_STEP=9`: the harness feels the world.** Each frame the humanoid harness drains the impulse the solver booked as refused by its hips rail and absorbs the horizontal part as a body of the walker's mass would; the vertical part is reported on the parts. Default off; known runaway with `PHYSICS_HZ=60` and a teleport.
- **`INV40_STEP=10`: the walk clip's toe-off lifts** (ankle and toe dorsiflexed) instead of pointing the toe into the floor. Default off.
...
- **Physics (single-law lever): torsion transmits only through touching
  rows, and the position pass stops at the error tolerance it meant.**
  A predicted-pose contact manifold could carry the torsional-friction
  row across an open gap (a dropped spinning cube lost its spin 51 mm
  above the floor); torsion now transmits only through rows within
  the engine's contact tolerance of touching. The position pass's
